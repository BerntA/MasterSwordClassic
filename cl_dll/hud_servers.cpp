...
void CHudServers::ListResponse( struct net_response_s *response )
{
	...
	if ( response->response )
	{
		...
		while ( list )
		{
			//Print( "%i - %i.%i.%i.%i\n", c, list->remote_address.ip[0], list->remote_address.ip[1], list->remote_address.ip[2], list->remote_address.ip[3]);
			c++;
			m_ServerListCount++;
...
void CHudServers::ServerResponse( struct net_response_s *response )
{
	...
	case NETAPI_REQUEST_DETAILS:
		if ( response->response )
		{
			...
			//Print( "Info: %s\n", szresponse );
			//logfile << "Svr: " << szresponse << "\r\n";
			const char *pszGameDir = NET_API->ValueForKey( szresponse, "gamedir" );
			//logfile << "Svr Mod: " << (pszGameDir?pszGameDir:"None?") << "\r\n";
			if( pszGameDir && stricmp(pszGameDir,"MS") ) break;
...
void CHudServers::Think( double time )
{
	...
	if ( m_fElapsed >= QUERY_TIMEOUT && !m_nQuerying )
	{
		CancelRequest( );
		return;
	}

	if( m_ServerListCount && m_ServersParsed >= m_ServerListCount )
	{
		CancelRequest( );
		return;
	}
...
void CHudServers::RequestList( void )
{
	...
	m_ServerListCount = m_ServersParsed = 0;

	int	count = 0;
	netadr_t adr[3];

	if ( !LoadMasterAddresses( 3, &count, adr ) )
	{
	...
	for( int i = 0; i < count; i++ )
		NET_API->SendRequest( context_id++, NETAPI_REQUEST_SERVERLIST, 0, QUERY_TIMEOUT, &adr[i], ::ListResponse );
}
...
double ServersGetTimeLeft( void )
{
	if ( g_pServers )
	{
		return max(QUERY_TIMEOUT-g_pServers->m_fElapsed,0);
	}
	return 0;
}

bool ServersIsListing( void )
{
	if ( g_pServers )
	{
		return g_pServers->m_nQuerying ? true : false;
	}
	return 0;
}
float ServersParsedPercent( void )
{
	if ( g_pServers )
	{
		return (g_pServers->m_ServersParsed/(float)g_pServers->m_ServerListCount) * 100.0;
	}
	return 0;
}
