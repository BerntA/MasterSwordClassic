	...
	public:
		...
		void DoRefresh(void)
		{
			...
			if (MSGlobals::IsLanGame)
				BroadcastServersList(0); //Request LAN listing
			else
				ServersList(); //Requst Internet listing
		}
		...
		void DoConnect(void)
		{
			...
			//gEngfuncs.Con_Printf( "Connecting to %s\n", address );
			 ...
			 _snprintf(sz, sizeof(sz),  "connect %s\n",  address );
		...
		virtual Panel *getCellRenderer(int column, int row, bool columnSelected, bool rowSelected, bool cellSelected)
		{
			...
			if (info)
			{
				...
				case 3:
					...
					if (val && val2)
					{
						 _snprintf(sz, sizeof(sz),  "%s/%s",  val,  val2 );
						sz[31] = '\0';
						// Server Map;
						m_pLabel->setText(sz);
					}
			...
			else
			{
				if (!row && !column)
				{
					if (ServersIsListing())
					{
						m_pLabel->setText("Filtering Master Sword servers... (%.1f%%)", ServersParsedPercent());
					}
					else if (ServersIsQuerying())
					{
						m_pLabel->setText("Waiting %.0fs for Server response...", ServersGetTimeLeft());
					}
					else
					{
...
public:
	...
	{
		m_pBrowser = pBrowser;
		 strncpy(m_szSortKey,  name, sizeof(m_szSortKey) );
	}
...
public:
	CSBLabel(char *name, char *sortkey) : Label(name)
	{
		m_pBrowser = NULL;
		 ...
		 strncpy(m_szSortKey,  sortkey, sizeof(m_szSortKey) );
