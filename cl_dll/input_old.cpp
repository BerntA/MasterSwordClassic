...
{
	KeyDown(&in_attack2);
	//__CmdFunc_InputPlayerSpecial();  Dogg: Not used in MS
}
...
void IN_JumpDown (void) 
{
	//Master Sword
	if( !IsPlayerActing( ) )
		KeyDown(&in_jump); //Don't jump while stting
}
...
{
	if( g_iVisibleMouse )	//Master Sword
		return;
...
extern int *g_pPlayerSkillPtr;
#define SKILL_ROGUE_FADE (1<<0)
...
{	
	...
	//if ( active )
	{
		...
		// clip to maxspeed
		spd = ClientSpeed;//gEngfuncs.GetClientMaxspeed();
		if ( spd != 0.0 )
		{
			if( ClientSpeed > 5000.0 )
			{
				logfile << "ERROR: SPEED EXCEEDS 5000.0\r\n";
				ClientSpeed = 5000.0;
			}
			...
			/*static float t = fmov;
			if( t != fmov )
				Print( "MaxSpeed: %f  MySpeed: %f\n", spd, fmov );*/
		}
		//Print( "MaxSpeed: %f  MySpeed: %f\n", spd, sqrt( (cmd->forwardmove*cmd->forwardmove) + (cmd->sidemove*cmd->sidemove) + (cmd->upmove*cmd->upmove) ) );
	...
	if( g_pPlayerSkillPtr && (*g_pPlayerSkillPtr)&SKILL_ROGUE_FADE )
		cmd->buttons |= IN_DUCK;
}
...
/*int	CL_IsDead( void )
{
	//Master Sword - iHealth is global now
	return ( g_flHealth <= 0 ) ? 1 : 0;
}*/
...
void InitInput (void)
{
	...
	cl_forwardspeed		= gEngfuncs.pfnRegisterVariable ( "cl_forwardspeed", "400", 0 );		//Master Sword - Don't save these
	cl_backspeed		= gEngfuncs.pfnRegisterVariable ( "cl_backspeed", "400", 0 );			//Master Sword - Don't save these
