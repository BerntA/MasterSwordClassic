...
void WeaponsResource :: SelectSlot( int iSlot, int fAdvance, int iDirection )
{
	 ...
	 for (int i = 0; i < player.Gear.size(); i++) 
	{
		...
		if(( pPack->m_Location == ITEMPOS_BACK  && !iSlot ) ||
			( pPack->m_Location == ITEMPOS_BELT  && iSlot == 1 ) ||
			( pPack->m_Location == ITEMPOS_SIDE  && iSlot == 2 ) ||
			( pPack->m_Location == ITEMPOS_SHOULDER  && iSlot == 3 ))
		{
			pPack->Container_AttackButtonDown( );
			ClientCmd( UTIL_VarArgs("inv open %i\n",pPack->m_iId ) );
			break;
		}
	}*/
}
