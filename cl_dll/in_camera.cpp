...
/*#ifdef LATER
	if ( cam_contain->value )
	{
		...
		VectorCopy( player->origin, origin );
		ext[0] = ext[1] = ext[2] = 0.0;
	}
#endif*/

	//Master Sword - set these in stone
	//camAngles[ PITCH ] = 10;//cam_idealpitch->value;
	//camAngles[ YAW ] = 0;//cam_idealyaw->value;
	//dist = 40;//30 //cam_idealdist->value;

	camAngles[ PITCH ] = cam_idealpitch->value;
	camAngles[ YAW ] = cam_idealyaw->value;


	//If switching to first person, then my ideal dist is zero so it zooms in
	dist = Cam_SwitchingTo1StPerson ? 0 : cam_idealdist->value;
	...
	if (cam_mousemove)
	{
	  ...
	  if (!cam_distancemove)
	  {
		...
		//Master Sword -- UNDONE - allow free movement
		...
		//Master Sword - Since you can move in a full circle now.. normalize the angles
		camAngles[ YAW ] += (CAM_ANGLE_MOVE)*((cam_mouse.x-gEngfuncs.GetWindowCenterX())/2);
		NormalizeAngles( (float *)&camAngles[YAW] );
...
/*#ifdef LATER
	if( cam_contain->value )
	{
		...
		clip.trace = *gEngfuncs.PM_TraceLine( (float *)&origin, (float *)&pnt, PM_TRACELINE_PHYSENTSONLY, 2, CLIgnoreFlags );
	...
	//Clip against walls
	cl_entity_t *ent = gEngfuncs.GetEntityByIndex( 0 );
	if( ent )
	{
		cl_entity_t *clPlayer = gEngfuncs.GetLocalPlayer();
		Vector Result;
		V_GetChaseOrigin( viewangles, clPlayer->origin + Vector(0,0,28), dist, Result );
		//cam_idealpitch->value = Result[ PITCH ];
		//cam_idealyaw->value = Result[ YAW ];
 		dist = v_lastDistance;
	}
	...
	else
	{
		...
		if( abs( camAngles[ 2 ] - dist ) < 2.0 )
		{
			camAngles[ 2 ] = dist;
			if( Cam_SwitchingTo1StPerson )
			{
				Cam_SwitchingTo1StPerson = false;
				MSCLGlobals::CamThirdPerson = false;
			}
		}
		else
			camAngles[ 2 ] += ( dist - camAngles[ 2 ] ) / 4.0;
	}
/*#ifdef LATER
	if( cam_contain->value )
	{
...
{ 
	...
	if( !MSCLGlobals::CamThirdPerson )
	{
		MSCLGlobals::CamThirdPerson = true; 
...
void CAM_Init( void )
{
	...
	c_mindistance			= gEngfuncs.pfnRegisterVariable ( "c_mindistance",   "20.0", 0 ); //Master Sword - Changed from 30
}
