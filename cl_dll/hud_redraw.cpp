...
int CHud :: DrawHudNumberSML( int x, int y, int iFlags, int iNumber, int r, int g, int b)
{
	...
	if( idigits >= 256 ) ConsolePrint( "ERROR: numofdigits( )\n" );
/*		if( iNumber < 3 ) { 
		char a[50];
	...
	itemp = idigits;
	while( itemp ) {
		if( itemp == idigits ) k = iNumber/pow(10,itemp-1);
		else if( itemp == 1 ) k = iNumber % 10;
		else k = (iNumber%(int)pow(10,itemp))/pow(10,itemp-1);
		k = min( k, 9 );
