...
CClassMenuPanel::CClassMenuPanel(int iTrans, int iRemoveMe, int x, int y, int wide, int tall) : CMenuPanel(iTrans, iRemoveMe, x, y, wide, tall)
{
	...
	for (int i = 0; i <= PC_RANDOM; i++)
	{
		...
		// Class button
		 _snprintf(sz, sizeof(sz),  "%s",  CHudTextMessage::BufferedLocaliseTextString(sLocalisedClasses[i]) );
		...
		// Create the Class Name Label
		 _snprintf(sz, sizeof(sz),  "#Title_%s",  sTFClassSelection[i] );
		...
		if (bShowClassGraphic)
		{
			for (int team = 0; team < 2; team++)
			{
				if (team == 1)
				{
					 _snprintf(sz, sizeof(sz),  "%sred",  sTFClassSelection[i] );
				}
				else
				{
					 _snprintf(sz, sizeof(sz),  "%sblue",  sTFClassSelection[i] );
				}
		...
		// Open up the Class Briefing File
		 _snprintf(sz, sizeof(sz),  "classes/short_%s.txt",  sTFClassSelection[i] );
...
void CClassMenuPanel::Update()
{
	...
	for (int i = 0; i <= PC_RANDOM; i++)
	{
		...
		char sz[256];
		 _snprintf(sz, sizeof(sz),  m_sPlayersOnTeamString,  iTotal );
