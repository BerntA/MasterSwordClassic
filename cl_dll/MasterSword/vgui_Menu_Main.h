...
public:
	...
	VGUI_MenuMain( Panel *pParent ) : VGUI_MenuBase( pParent )
	{
		...
		struct btninfo_t { const char *Name; int Width; int OptionScreen; }
	...
	void Update()
	{
		...
		{
			if( vote_t::VotesTypesAllowed[i] == "kick" )	m_Buttons[OPT_VOTEKICK]->setEnabled( true );
			if( vote_t::VotesTypesAllowed[i] == "advtime" ) m_Buttons[OPT_VOTETIME]->setEnabled( true );
		}
