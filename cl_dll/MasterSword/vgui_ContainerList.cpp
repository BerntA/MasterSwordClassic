...
public:
	...
	{
		if( code == MOUSE_LEFT ) fBlockVGUIMouseButton1 = false;
	}
...
void CContainerPanel::GearItemSelected( ulong ID )
{
	...
	if( ID > 0 )
	{
		...
		if( pGearItem ) Title = pGearItem->DisplayName( );
	}
	else if( !ID ) Title = "Player hands";
...
bool CContainerPanel::GearItemDoubleClicked( ulong ID )
{
	...
	if( !pWornItem ) return false;

	if( !pWornItem->IsWorn() ) return false;
...
void CContainerPanel::Open( void )
{
	...
	int vKey = VK_LBUTTON;
	if( GetSystemMetrics(SM_SWAPBUTTON) ) vKey = VK_RBUTTON;
	if( HIWORD(GetAsyncKeyState( vKey )) ) fBlockVGUIMouseButton1 = true;
	else fBlockVGUIMouseButton1 = false;
	 ...
	 for (int i = 0; i < m_GearPanel->GearItemButtonTotal; i++) 
	{
		if( player.HasConditions( MONSTER_OPENCONTAINER ) )
		{
			if( m_OpenContainerID == m_GearPanel->GearItemButtons[i]->m_GearItemID )
				{ m_GearPanel->Select( i ); break; }
		}
		else { m_GearPanel->Select( i ); break; }	//If no specific containter open, just select the first one - Player hands
	}
}
