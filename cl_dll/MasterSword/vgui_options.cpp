...
{
public:
	VoteButton( Panel *pParent, msstring_ref Text, int x, int y, int wide, int tall ) :
	  MSButton( pParent, Text, x, y, wide, tall, Color_BtnHighlight, Color_FadedWhite ) { }
};
...
public:
	...
	enum actoption_e { opt_vote, opt_joinparty, opt_createparty } m_OptionType;

	CAction_OptionBegin( VGUI_Options *pPanel, actoption_e OptionType ) { m_Panel = pPanel; m_OptionType = OptionType; }
...
public:
	...
	virtual void actionPerformed( Panel* panel )
	{
		if( m_Type == 0 )		ClientCmd( "vote 1\n" );
		else if( m_Type == 1 )	ClientCmd( "leaveparty\n" );
		ShowVGUIMenu( 0 );
	}
};
...
public:
	...
	{
		m_Target = idx;
		for (int i = 0; i < MAX_VOTE_PLAYERS; i++) m_pListButton[i]->SetBgColor(Color_Transparent);
...
void CPanel_Options::Open( option_e OptionScreen )
{
	...
	case OPTIONSC_VOTETIME:
		if( !MSGlobals::CurrentVote.fActive ) m_pInitVote->Open( (m_OpenScreen == OPTIONSC_VOTEKICK) ? "kick" : "advtime" );
		else m_pCastVote->Open( );
		break;
	case OPTIONSC_PARTY:
		if( !player.TeamID()[0] )
			m_pPartyPanel->Open( );
...
int __MsgFunc_Vote(const char *pszName, int iSize, void *pbuf)
{
	...
	switch( VoteActive )
	{
		...
		case 1:  //Start the vote
		{
			...
			else strcpy( cSourcePlayerName, "<unknown>" );
			...
			if( MSGlobals::CurrentVote.Type == "kick" && Params.size() >= 3 )
			{
				...
				else strcpy( cTargetPlayerName, "<unknown>" );
