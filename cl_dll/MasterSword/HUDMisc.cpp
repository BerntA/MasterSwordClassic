...
{
	player.m_SayType++;
	if( player.m_SayType > SPEECH_PARTY ) player.m_SayType = SPEECH_GLOBAL;

	msstring SayString = "You ";
	switch( player.m_SayType )
	{
	case SPEECH_GLOBAL: SayString += "begin to shout!\n";					break;
	case SPEECH_PARTY:	SayString += "aim your voice toward your party.\n"; break;
	default:			SayString += "speak normally.\n";					break;
	}
...
void CHudMisc :: UserCmd_RemovePack( void )
{
	...
	if( gHUD.m_Menu->HideMyMenu( MENU_REMOVEPACK ) ) return;
...
void CHudMisc :: UserCmd_Offer( void )
{
	...
	//Menu is already on, turn it off
	if( gHUD.m_Menu->HideMyMenu( MENU_OFFER ) )  return;
	if( gHUD.m_iHideHUDDisplay&HIDEHUD_ALL ) return;
	...
	if( EntInfo.Type > ENT_FRIENDLY ) return;
