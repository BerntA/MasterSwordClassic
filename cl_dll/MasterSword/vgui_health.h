...
public:
	...
	void Update( )
	{
		...
		//[/Thothie]
 		...
 		if( m_CurrentAmt < Amt ) m_CurrentAmt += min(MaxChange,Amt - m_CurrentAmt);
		else if( m_CurrentAmt > Amt ) m_CurrentAmt -= min(MaxChange,m_CurrentAmt - Amt);
...
public:
	...
	{ 
		...
		else m_pStamina->SetFGColorRGB( HighColor ); 
