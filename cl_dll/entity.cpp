...
*/
	...
	if( gHUD.m_HUDScript ) gHUD.m_HUDScript->Effects_PreRender( );
...
{
	if( !g_CurrentTempEnt ) RETURN_NOTHING;
	TEMPENTITY *p = g_CurrentTempEnt;

	static msstring Return;
	if( Prop == "deathtime" ) RETURN_FLOAT( p->die )
	else if( Prop == "bouncefactor" ) RETURN_FLOAT( p->bounceFactor )
	else
	{
		static msstringlist Params;
		Params.clearitems();
...
{ \
	if( Prop == name ) { Vector vTemp = Vector(color.r,color.g,color.b); return (Return = VecToString( vTemp )); }  \
	else if( Prop == name ".r" )	RETURN_FLOAT( color.r ) \
	else if( Prop == name ".g" )	RETURN_FLOAT( color.g ) \
	else if( Prop == name ".b" )	RETURN_FLOAT( color.b ) \
}
...
msstring_ref CScript::CLGetBeamProp( int beamid, msstringlist &Params )
{
	...
	*/
	...
	if ( found_beam )
	{
		...
		if ( Prop == "start" ) beamret = VecToString(pBeam->source);
		else if ( Prop == "end" ) beamret = VecToString(pBeam->target);
		else if ( Prop == "delta" ) beamret = VecToString(pBeam->delta);
		else if ( Prop == "width" ) beamret = FloatToString(pBeam->width);
		else if ( Prop == "amplitude" ) beamret = FloatToString(pBeam->amplitude);
		else if ( Prop == "color" )
		{
			beamret = UTIL_VarArgs("(%f,%f,%f)",pBeam->r,pBeam->g,pBeam->b);
		}
		else if ( Prop == "brightness" ) beamret = FloatToString(pBeam->brightness);
		else if ( Prop == "speed" ) beamret = FloatToString(pBeam->speed);
		else if ( Prop == "life" ) beamret = FloatToString(pBeam->die);
		else if ( Prop == "segments" ) beamret = IntToString(pBeam->segments);
		else if ( Prop == "framerate" ) beamret = FloatToString(pBeam->frameRate);
		else if ( Prop == "framerate" ) beamret = IntToString(pBeam->frameCount);
		else if ( Prop == "sprite" ) beamret = IntToString(pBeam->modelIndex);
		else if ( Prop == "startent" ) beamret = IntToString(pBeam->startEntity);
		else if ( Prop == "endent" ) beamret = IntToString(pBeam->endEntity);
		else if ( Prop == "flags" ) beamret = IntToString(pBeam->flags);
		else if ( Prop == "t" ) beamret = FloatToString(pBeam->t);
		return beamret;
	}
	else
	{
...
{
	if( !pclEntity ) RETURN_NOTHING;
	cl_entity_t &ent = *pclEntity;
	static msstring Return;
	msstring &Prop = Params[1];
	if( Prop.starts_with("origin") ) RETURN_POSITION( "origin", ent.origin )
	else if( Prop.starts_with("center") ) { Vector vecCenter = ent.curstate.origin + ((ent.curstate.mins + ent.curstate.maxs) * 0.5); RETURN_POSITION( "center", vecCenter ) }
	else if( Prop.starts_with("angles") ) RETURN_ANGLE( "angles", ent.angles )
	else if( Prop.starts_with("velocity") ) RETURN_POSITION( "velocity", ent.baseline.vuser1 )		//can't use baseline.origin - it's used by networked ents
		...
		return 
		(EngineFunc::Shared_PointContents( Vector(ent.origin.x,ent.origin.y,ent.origin.z+ent.curstate.mins.z) ) == CONTENTS_WATER
		&& EngineFunc::Shared_PointContents( Vector(ent.origin.x,ent.origin.y,ent.origin.z+ent.curstate.maxs.z) ) == CONTENTS_WATER) ? "1" : "0";
	...
	else if( Prop == "model" )
	{
		if( ent.model )
		{
			...
			if( Return.starts_with( "models/" ) ) Return = Return.substr( 7 );
			if( Return.starts_with( "sprites/" ) ) Return = Return.substr( 8 );
			return Return;
		}
	...
	*/
	else if( Prop == "ducking" ) return FBitSet(player.pev->flags, FL_DUCKING) ? "1" : "0"; //Thothie JUN2010_25
	else if( Prop == "modelidx" ) RETURN_INT( ent.curstate.modelindex )
	else if( Prop == "anim" ) RETURN_INT( ent.curstate.sequence )
	else if( Prop == "frame" ) RETURN_INT( ent.curstate.frame )
	else if( Prop == "framerate" ) RETURN_INT( ent.curstate.framerate )
	else if( Prop == "exists" ) return pclEntity->Exists( ) ? "1" : "0";
	else if( Prop == "gravity" ) RETURN_FLOAT( ent.curstate.gravity )
	else if( Prop == "scale" ) RETURN_FLOAT( ent.curstate.scale )
	//NOV2014_16 dealing with the fact that MSC scripts can't hanle percision more than 2f
	else if( Prop == "scaleHD" ) RETURN_FLOAT( ent.curstate.scale * 1000 )
	else if( Prop == "fuser1HD" ) RETURN_FLOAT( ent.curstate.fuser1 * 1000 )
	else if( Prop == "renderfx" ) RETURN_INT( ent.curstate.renderfx )
	else if( Prop == "rendermode" ) RETURN_INT( ent.curstate.rendermode )
	else if( Prop == "renderamt" ) RETURN_FLOAT( ent.curstate.renderamt )
	else if( Prop.starts_with("rendercolor") ) RETURN_COLOR( "rendercolor", ent.curstate.rendercolor )
	else if( Prop == "visible" ) return FBitSet(ent.curstate.effects,EF_NODRAW) ? "0" : "1";
	else if( Prop == "isplayer" ) return ent.player ? "1" : "0";
	else if( Prop.starts_with("attachment0") ) RETURN_POSITION( "attachment0", ent.attachment[0] )
	else if( Prop.starts_with("attachment1") ) RETURN_POSITION( "attachment1", ent.attachment[1] )
	else if( Prop.starts_with("attachment2") ) RETURN_POSITION( "attachment2", ent.attachment[2] )
	else if( Prop.starts_with("attachment3") ) RETURN_POSITION( "attachment3", ent.attachment[3] )
	else if( Prop == "iuser1" ) RETURN_INT( ent.curstate.iuser1 )
	else if( Prop == "iuser2" ) RETURN_INT( ent.curstate.iuser2 )
	else if( Prop == "iuser3" ) RETURN_INT( ent.curstate.iuser3 )
	else if( Prop == "iuser4" ) RETURN_INT( ent.curstate.iuser4 )
	else if( Prop == "fuser1" ) RETURN_FLOAT( ent.curstate.fuser1 )
	else if( Prop == "fuser2" ) RETURN_FLOAT( ent.curstate.fuser2 )
	else if( Prop == "fuser3" ) RETURN_FLOAT( ent.curstate.fuser3 )
	else if( Prop == "fuser4" ) RETURN_FLOAT( ent.curstate.fuser4 )
	else if( Prop == "prevstate.iuser1" ) RETURN_INT( ent.prevstate.iuser1 )
	else if( Prop == "prevstate.iuser2" ) RETURN_INT( ent.prevstate.iuser2 )
	else if( Prop == "prevstate.iuser3" ) RETURN_INT( ent.prevstate.iuser3 )
	else if( Prop == "prevstate.iuser4" ) RETURN_INT( ent.prevstate.iuser4 )
	else if( Prop == "prevstate.fuser1" ) RETURN_FLOAT( ent.prevstate.fuser1 )
	else if( Prop == "prevstate.fuser2" ) RETURN_FLOAT( ent.prevstate.fuser2 )
	else if( Prop == "prevstate.fuser3" ) RETURN_FLOAT( ent.prevstate.fuser3 )
	else if( Prop == "prevstate.fuser4" ) RETURN_FLOAT( ent.prevstate.fuser4 )
	else if( Prop == "baseline.iuser1" ) RETURN_INT( ent.baseline.iuser1 )		//Baseline props are not overriden by
	else if( Prop == "baseline.iuser2" ) RETURN_INT( ent.baseline.iuser2 )		//incoming server packets
	else if( Prop == "baseline.iuser3" ) RETURN_INT( ent.baseline.iuser3 )		//Stuff stored here is persistent
	else if( Prop == "baseline.iuser4" ) RETURN_INT( ent.baseline.iuser4 )
	else if( Prop == "baseline.fuser1" ) RETURN_FLOAT( ent.baseline.fuser1 )
	else if( Prop == "baseline.fuser2" ) RETURN_FLOAT( ent.baseline.fuser2 )
	else if( Prop == "baseline.fuser3" ) RETURN_FLOAT( ent.baseline.fuser3 )
	else if( Prop == "baseline.fuser4" ) RETURN_FLOAT( ent.baseline.fuser4 )
...
void CScript::CLScriptedEffect( msstringlist &Params )
{
	...
	if( Params[0] == "tempent" )
	{
		...
		else if( Params[1] == "set_current_prop" )
		{
			...
			if( Cmd == "death_delay" )
			{
				if( Value == "last_frame" )  ClearBits( p->flags, FTENT_SPRANIMATELOOP );
				...
				else p->die = gEngfuncs.GetClientTime() + atof(Value);
			}
			else if( Cmd == "bouncefactor" ) p->bounceFactor = atof(Value);
			else if( Cmd == "framerate" )
			{
				SetBits( p->flags, FTENT_SPRANIMATE );
				p->entity.curstate.framerate = atof(Value);
			}
			else if( Cmd == "frames" ) p->frameMax = atof(Value);
			else if( Cmd == "collide" )
			{
				...
				else if( Value.contains("world") ) { ClearBits( p->flags, FTENT_COLLIDEALL ); SetBits( p->flags, FTENT_COLLIDEWORLD ); }
				else if( Value.contains("none") ) ClearBits( p->flags, FTENT_COLLIDEALL|FTENT_COLLIDEWORLD );
				
				//Not else
				if( Value.contains("die") ) SetBits( p->flags, FTENT_COLLIDEKILL );
			}
	...
	else if( Params[0] == "frameent" )
	{
		...
		if( Options.contains("model") || Options.contains("sprite") )
		{
			...
			if( Model )
			{
				...
				if( CurrFrameEnt >= 256 ) CurrFrameEnt = 0;
				if( !IsPerm ) 
	...
	else if( Params[0] == "light" )
	{
		...
		if ( Params[1] == "new" && atoi(EngineFunc::CVAR_GetString("r_dynamic")) == 0 ) return;
		...
		if( Flags.contains("entity") ) EntityLight = true;
		if( Flags.contains("dark") ) NewLight.dark = true;
		...
		else if ( Params.size() >= 3 )
		{
			...
			*/
			if ( found_beam && Params.size() >= 4 )
			{
				...
				if ( Params[2] == "start" ) pBeam->source = StringToVec(Params[3]);
				else if ( Params[2] == "end" ) pBeam->target = StringToVec(Params[3]);
				else if ( Params[2] == "delta" ) pBeam->delta = StringToVec(Params[3]); //whatever "delta" is
				else if ( Params[2] == "width" ) pBeam->width = atof(Params[3]);
				else if ( Params[2] == "amplitude" ) pBeam->amplitude = atof(Params[3]);
				else if ( Params[2] == "color" )
				{
					Vector col = StringToVec(Params[3]);
					pBeam->r = col.x; pBeam->g = col.y; pBeam->b = col.z;
				}
				else if ( Params[2] == "brightness" ) pBeam->brightness = atof(Params[3]);
				else if ( Params[2] == "speed" ) pBeam->speed = atof(Params[3]);
				else if ( Params[2] == "life" ) pBeam->die = atof(Params[3]);
				else if ( Params[2] == "framerate" ) pBeam->frameRate = atof(Params[3]);
				else if ( Params[2] == "framerate" ) pBeam->frameCount = atoi(Params[3]); //need this if you change sprites
				//else if ( Params[2] == "segments" ) pBeam->segments = atoi(Params[3]); //no affect
				//else if ( Params[2] == "t" ) pBeam->t = atof(Params[3]); //no affect
				else if ( Params[2] == "sprite" ) pBeam->modelIndex = CL_LoadModel( Params[3] );
				else if ( Params[2] == "startent" ) (Params.size()>=5) ?  pBeam->startEntity = ( atoi(Params[3]) & 0x0FFF ) | (( atoi(Params[4]) & 0xF ) << 12) : pBeam->startEntity = atoi(Params[3]);
				else if ( Params[2] == "endent" ) (Params.size()>=5) ? pBeam->endEntity = ( atoi(Params[3]) & 0x0FFF ) | (( atoi(Params[4]) & 0xF ) << 12) : pBeam->endEntity = atoi(Params[3]);
				else if ( Params[2] == "remove" ) pBeam->die = 0; //case we got extra pars due to pass params
			}
	...
	else if ( Params[0] == "spark" )
	{
		...
		if( Params[1].starts_with("(") ) gEngfuncs.pEfxAPI->R_SparkEffect(StringToVec(Params[1]),8, -200, 200 );
		else
		{
...
void SetClEntityProp( cl_entity_t &Ent, msstring &Cmd, mslist<msstring *> &Params )
{
	...
	if( Cmd == "origin" ) Ent.origin = Ent.curstate.origin = StringToVec( Value );
	else if( Cmd == "origin.x" ) Ent.origin.x = atof( Value );
	else if( Cmd == "origin.y" ) Ent.origin.y = atof( Value );
	else if( Cmd == "origin.z" ) Ent.origin.z = atof( Value );
	else if( Cmd == "velocity" ) Ent.baseline.origin = StringToVec( Value );	//Velocity is stored in entity.baseline.origin... why? ASK VALVE
	else if( Cmd == "velocity.x" ) Ent.baseline.origin.x = atof( Value );
	else if( Cmd == "velocity.y" ) Ent.baseline.origin.y = atof( Value );
	else if( Cmd == "velocity.z" ) Ent.baseline.origin.z = atof( Value );
	else if( Cmd == "angles" )			Ent.angles = StringToVec( Value );
	else if( Cmd == "angles.pitch" )	Ent.angles.x = atof( Value );
	else if( Cmd == "angles.yaw" )		Ent.angles.y = atof( Value );
	else if( Cmd == "angles.roll" )		Ent.angles.z = atof( Value );
	...
	else if( Cmd == "maxs" )		Ent.curstate.maxs = StringToVec( Value );
	else if( Cmd == "mins" )		Ent.curstate.mins = StringToVec( Value );
	else if( Cmd == "gravity" )		Ent.curstate.gravity = atof(Value);
	else if( Cmd == "owner" )		Ent.curstate.owner = atoi(Value);
	else if( Cmd == "follow" )
	{
		if( Value != "none" )
		{
	...
	else if( Cmd == "body" ) Ent.curstate.body = atoi(Value);
	else if( Cmd == "skin" ) Ent.curstate.skin = atoi(Value);
	else if( Cmd == "aiment" ) Ent.curstate.aiment = atoi(Value);
	else if( Cmd == "movetype" ) Ent.curstate.movetype = atoi(Value);
	else if( Cmd == "scale" ) Ent.curstate.scale = atof(Value);
	//dealing with the fact that MSC can't handle floats more than 2f
	else if( Cmd == "scaleHD" ) Ent.curstate.scale = atof(Value)*0.001;
	else if( Cmd == "fuser1HD" ) Ent.curstate.fuser1 = atof(Value)*0.001;
	else if( Cmd == "framerate" ) Ent.curstate.framerate = atof(Value);
	else if( Cmd == "frame" ) Ent.curstate.frame = atof(Value);
	else if( Cmd == "update" ) Ent.curstate.iuser4 = atoi(Value) ? SetBits(Ent.curstate.iuser4,MSTEMPENT_CALLBACK) : ClearBits(Ent.curstate.iuser4,MSTEMPENT_CALLBACK);
	else if( Cmd == "rendermode" )
	{
		if( Value.find("normal") != msstring_error )		Ent.curstate.rendermode = kRenderNormal;
		else if( Value.find("color") != msstring_error )	Ent.curstate.rendermode = kRenderTransColor;
		else if( Value.find("texture") != msstring_error )	Ent.curstate.rendermode = kRenderTransTexture;
		else if( Value.find("glow") != msstring_error )		Ent.curstate.rendermode = kRenderGlow;
		else if( Value.find("alpha") != msstring_error )	Ent.curstate.rendermode = kRenderTransAlpha;
		else if( Value.find("add") != msstring_error )		Ent.curstate.rendermode = kRenderTransAdd;
	}
	else if( Cmd == "renderamt" ) Ent.curstate.renderamt = atof(Value);
	else if( Cmd == "renderfx" )
	{
		if( Value.find("normal") != msstring_error )		Ent.curstate.renderfx = kRenderFxNone;
		else if( Value.find("glow") != msstring_error )		Ent.curstate.renderfx = kRenderFxGlowShell;
		else if( Value.find("player") != msstring_error )	Ent.curstate.rendermode = kRenderFxDeadPlayer;
	}
	...
	else if( Cmd == "sequence" ) Ent.curstate.sequence = atoi(Value); //FEB2009_19
	else if( Cmd == "iuser1" ) Ent.curstate.iuser1 = atoi(Value);
	else if( Cmd == "iuser2" ) Ent.curstate.iuser2 = atoi(Value);
	else if( Cmd == "iuser3" ) Ent.curstate.iuser3 = atoi(Value);
	else if( Cmd == "iuser4" ) Ent.curstate.iuser4 = atoi(Value);
	else if( Cmd == "fuser1" ) Ent.curstate.fuser1 = atof(Value);
	else if( Cmd == "fuser2" ) Ent.curstate.fuser2 = atof(Value);
	else if( Cmd == "fuser3" ) Ent.curstate.fuser3 = atof(Value);
	else if( Cmd == "fuser4" ) Ent.curstate.fuser4 = atof(Value);
	else if( Cmd == "prevstate.iuser1" ) Ent.prevstate.iuser1 = atoi(Value);
	else if( Cmd == "prevstate.iuser2" ) Ent.prevstate.iuser2 = atoi(Value);
	else if( Cmd == "prevstate.iuser3" ) Ent.prevstate.iuser3 = atoi(Value);
	else if( Cmd == "prevstate.iuser4" ) Ent.prevstate.iuser4 = atoi(Value);
	else if( Cmd == "prevstate.fuser1" ) Ent.prevstate.fuser1 = atof(Value);
	else if( Cmd == "prevstate.fuser2" ) Ent.prevstate.fuser2 = atof(Value);
	else if( Cmd == "prevstate.fuser3" ) Ent.prevstate.fuser3 = atof(Value);
	else if( Cmd == "prevstate.fuser4" ) Ent.prevstate.fuser4 = atof(Value);
	else if( Cmd == "baseline.iuser1" ) Ent.baseline.iuser1 = atoi(Value);
	else if( Cmd == "baseline.iuser2" ) Ent.baseline.iuser2 = atoi(Value);
	else if( Cmd == "baseline.iuser3" ) Ent.baseline.iuser3 = atoi(Value);
	else if( Cmd == "baseline.iuser4" ) Ent.baseline.iuser4 = atoi(Value);
	else if( Cmd == "baseline.fuser1" ) Ent.baseline.fuser1 = atof(Value);
	else if( Cmd == "baseline.fuser2" ) Ent.baseline.fuser2 = atof(Value);
	else if( Cmd == "baseline.fuser3" ) Ent.baseline.fuser3 = atof(Value);
	else if( Cmd == "baseline.fuser4" ) Ent.baseline.fuser4 = atof(Value);
}
...
void DLLEXPORT HUD_StudioEvent( const struct mstudioevent_s *event, const struct cl_entity_s *entity )
{
	...
	case 8500:
		if( event->options )
		{
			if( !stricmp(event->options,"twohandanim") )		ViewModel_InactiveModelVisible( true, entity );
			else if( !stricmp(event->options,"onehandanim") )	ViewModel_InactiveModelVisible( false, entity );
		}
		break;
	default:
		break;
				...
				if ( pTemp->flags & FTENT_COLLIDEALL )
				{
					...
					if ( pmtrace.fraction != 1 )
					{
						...
						if (  pTemp->flags & (FTENT_SKIPENT)  )
						{
							...
							if ( iSkipent && pe->info == iSkipent ) skip_ent = true;
						}
						if ( !skip_ent ) 
						{
