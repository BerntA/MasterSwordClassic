...
public:
	...
	//Flip models across the x axis
	//virtual void FlipModel( bool Enable );

	//Dynamically modify a bone
	void ModifyBone( mstudiobone_t *pbone, Vector &angle );

	//Register the extra data slot.  Holds various MS-specific data
	void RegisterExtraData( cl_entity_t &Ent );

	//Setup the renderer just before rendering the model
	void StudioSetupRender( bool Setup );

	enum { DRAW_DEFAULT, DRAW_BACKFACES, DRAW_BOTHFACES } m_DrawStyle;
...
public:
	...
	float			m_FlipMatrix[3][4];
};

//Extra model info, attached to each model
//A model's extra info is zero'd the first time the model is rendered
struct modelinfo_t
{
	bool Used;
	int Bones;
	Vector InterpOrg, InterpAng;			//Interpolated position and angles for this frame
	Vector BoneOrigins[MAXSTUDIOBONES];
	float BoneTransformations[MAXSTUDIOBONES][ 3 ][ 4 ];
	bool InterpolateFromFullRotation;
};
class CModelMgr
{
public:
