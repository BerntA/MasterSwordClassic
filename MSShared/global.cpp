...
int EntityNameToType(const char *pszName)  { for (int i = 0; i < ENT_TYPE_TOTAL; i++) if (!stricmp(pszName, g_EntTypeByName[i])) return i; return -1; }

entityinfo_t::entityinfo_t( CBaseEntity *pEntity ) { entindex = pEntity->entindex(); pvPrivData = pEntity; }
entityinfo_t& entityinfo_t::operator = (CBaseEntity *pEntity) { entindex = (pEntity ? pEntity->entindex() : 0); pvPrivData = pEntity; return *this; }
...
bool UnderSky ( Vector Origin )
{
	...
	if ( !Tr.Allsolid )
	{
		if ( EngineFunc::Shared_PointContents( Tr.EndPos ) == CONTENTS_SKY ) return true;
	}
