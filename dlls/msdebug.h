...
//string_i - In debug mode, a normal string.  In release mode its created within the engine and released each level
	...
	//#endif
		...
		struct string_i
		{
			...
			msstring_ref operator = ( msstring_ref a )	{ m_string = a; return m_string; }
			msstring_ref operator = ( const msstring &a )	{ m_string = a; return m_string; }
...
struct memalloc_t
{
	...
	void *pAddr;
	bool Used;
	inline memalloc_t &operator = ( const memalloc_t &mem ) { SourceFile = mem.SourceFile; LineNum = mem.LineNum; pAddr = mem.pAddr; return *this; }
};
