...
class CSubStat {
public:
	CSubStat( int iValue, int iExp ) { Value = iValue; Exp = iExp; }
	CSubStat( ) { Clear( ); }
	~CSubStat( );
	void Clear( ) { Value = 0; Exp = 0; }
...
public:
	...
	string_i m_Name;
	enum skilltype_e { STAT_NAT, STAT_SKILL } m_Type;	//Stat type

	CStat( ) { } 
	CStat( msstring_ref Name, skilltype_e Type ) { m_Name = Name; m_Type = Type; } 
