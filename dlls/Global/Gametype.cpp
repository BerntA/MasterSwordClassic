...
CBaseEntity *GetAssaultSpawnSpot(CBasePlayer *pPlayer)
{
	...
	if (!pPlayer->Race)
		RaceID = RANDOM_LONG(0, RACE_NUM - 1);
	else
		RaceID = pPlayer->Race->id;

	if (RaceID == RACE_HUMAN)
		SpawnSpotName = "playerspawn_human";
	else if (RaceID == RACE_DWARF)
		SpawnSpotName = "playerspawn_dwarf";
	else if (RaceID == RACE_ELF)
		SpawnSpotName = "playerspawn_elf";
	...
	while (pSpot != pFirstSpot)
	{
		...
		if (pSpot && IsSpawnPointValid(pPlayer, pSpot) && RANDOM_LONG(0, 1))
			break;
		n++;
		if (n > 255)
		{
		...
		// increment pSpot
		if (!pSpot)
			pSpot = UTIL_FindEntityByClassname(pFirstSpot, SpawnSpotName);
		else
			pSpot = UTIL_FindEntityByClassname(pSpot, SpawnSpotName);
	}
	...
	if (pSpot == pFirstSpot && !IsSpawnPointValid(pPlayer, pSpot))
	{
		...
		while (!bCanBreak && i < 100)
		{
			...
			vecEnd = pSpot->pev->origin + vDir * 250;
		...
		if (bCanBreak)
		{
			...
			ALERT(at_console, "Sorry, there's no where to spawn!!!\n");
	}
	return pSpot;
}