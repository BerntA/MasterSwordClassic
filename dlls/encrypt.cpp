...
void CEncryptData1::Encrypt( )
{
	...
	for( size_t i = 0; i < m_DataSize; i++ )
	{
		...
		if( Action ) CheckSum += (m_pData[i] * 2);
		else CheckSum -= m_pData[i];
	}
...
bool CEncryptData1::Decrypt( )
{
	...
	{
		bool Action = (i % 2) ? true : false;
		if( Action ) CheckSum += (Decrypted[i] * 2);
		else CheckSum -= Decrypted[i];
	}
...
void CEncryptData2::Encrypt( )
{
	...
	for( i = 0; i < usableDataSize; i++ ) m_pData[i] += rand();

}

//#include "logfile.h"
#define Foreach( var, max ) for( var = 0; var < (signed)max; var++ )	//variable declared

bool CEncryptData2::Decrypt( )
{
	long usableDataSize = m_DataSize - MS_SIZE_LONG;

	if( usableDataSize < 4 ) return false;
	...
	for( i = 0; i < loops; i++ ) randNum[i] = rand()%usableDataSize;
	for( i = loops-1; i >= 0; i-- )
	{
		num = randNum[i];
		tmp = m_pData[num];
