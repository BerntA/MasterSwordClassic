...
inline uint32 F( uint32 xx, uint32 yy, uint32 zz ) 
{  return (( xx & yy ) | (~xx & zz ));  }

inline uint32 G( uint32 xx, uint32 yy, uint32 zz ) 
{  return (( xx & zz ) | ( yy & ~zz )); }

inline uint32 H( uint32 xx, uint32 yy, uint32 zz )
{  return xx ^ yy ^ zz; }

inline uint32 I( uint32 xx, uint32 yy, uint32 zz ) 
{  return yy ^ ( xx | ~zz ); }

// ROTATE_LEFT rotates x left n bits.
inline uint32 ROTATE_LEFT( uint32 xx, int nn )
{  return ( xx << nn ) | ( xx >> (32-nn )); }

// FF, GG, HH, and II transformations for rounds 1, 2, 3, and 4.
// Rotation is separate from addition to prevent recomputation.
inline void 
FF(uint32 &aa, uint32 bb, uint32 cc, uint32 dd, uint32 xx, int ss, uint32 ac){ 
   aa += F( bb, cc, dd ) + xx + ac;
...
inline void
GG(uint32 &aa, uint32 bb, uint32 cc, uint32 dd, uint32 xx, int ss, uint32 ac){ 
   aa += G( bb, cc, dd ) + xx + ac;
   aa = ROTATE_LEFT( aa, ss );
   aa += bb;
   return; 
...
inline void 
HH(uint32 &aa, uint32 bb, uint32 cc, uint32 dd, uint32 xx, int ss, uint32 ac){ 
   aa += H( bb, cc, dd ) + xx + ac;
   aa = ROTATE_LEFT( aa, ss );
   aa += bb;
   return; 
...
inline void
II(uint32 &aa, uint32 bb, uint32 cc, uint32 dd, uint32 xx, int ss, uint32 ac){ 
   aa += I( bb, cc, dd ) + xx + ac;
   aa = ROTATE_LEFT( aa, ss );
   aa += bb;
   return; 
...
{
   stream.read( (char *)digest.m_data, MD5_DIGEST_LENGTH );
...
{
   stream.write( (char *)digest.m_data, MD5_DIGEST_LENGTH );
//   for( int ii = 0; ii < MD5_DIGEST_LENGTH; ii++ ) {
//      stream << digest.m_data[ii];
//   }
   return stream;
...
void
md5::update( const uint8 *input, size_t inputLen )
...
void
md5::transform( const uint8 block[64] )
{  // MD5 basic transformation. Transforms state based on block.
   ...
   uint32 aa = m_state[0], bb = m_state[1], cc = m_state[2], dd = m_state[3];
   uint32 xx[16];
   ...
   memset((uint8*)xx, 0, sizeof (xx));  // Clear sensitive information.
   return;
}

void
md5::encode( uint8  *output, const uint32 *input, int len )
{  // Encodes input (uint32) into output (uint8).
   // Assumes len is a multiple of 4.
  int ii, jj;
  for( ii = 0, jj = 0; jj < len; ii++, jj += 4 ) {
     output[jj]   = (uint8)(  input[ii]         & 0xFF );
     output[jj+1] = (uint8)(( input[ii] >>  8 ) & 0xFF );
     output[jj+2] = (uint8)(( input[ii] >> 16 ) & 0xFF );
     output[jj+3] = (uint8)(( input[ii] >> 24 ) & 0xFF );
  }
  return;
}
...
{  // Encodes m_state into digest.
  ...
  for( ii = 0, jj = 0; jj < MD5_DIGEST_LENGTH; ii++, jj += 4 ) {
     digest.m_data[jj]   = (uint8)(  m_state[ii]         & 0xFF );
     digest.m_data[jj+1] = (uint8)(( m_state[ii] >>  8 ) & 0xFF );
     digest.m_data[jj+2] = (uint8)(( m_state[ii] >> 16 ) & 0xFF );
     digest.m_data[jj+3] = (uint8)(( m_state[ii] >> 24 ) & 0xFF );
  }
  return;
}

void
md5::decode( uint32 *output, const uint8  *input, int len )
{  // Decodes input (uint8) into output (uint32). 
   // Assumes len is a multiple of 4.
  int ii, jj;
  for( ii = 0, jj = 0; jj < len; ii++, jj += 4 ) {
     output[ii] = ((uint32)input[jj]) | 
                 (((uint32)input[jj+1]) <<  8 ) |
                 (((uint32)input[jj+2]) << 16 ) | 
                 (((uint32)input[jj+3]) << 24 );
  }
  return;
}
