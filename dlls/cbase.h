...
enum givetype_e { GIVE_HP, GIVE_MP, GIVE_GOLD };

struct entityinfo_t { //For keeping track of an entity across frames
	entityinfo_t( ) { }
...
public:
	...
	inline float BaseThinkTime( ) { return m_Brush ? pev->ltime : gpGlobals->time; }
...
//			ALERT( at_error, "No EXPORT: %s:%s (%08lx)\n", STRING(pev->classname), name, (unsigned long)pFunction );
	...
	edict_t *edict() { //Master Sword... Client has no edict_t !
	#ifdef VALVE_DLL
			return ENT( pev ); 
	#else
