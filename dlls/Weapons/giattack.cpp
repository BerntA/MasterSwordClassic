...
bool CGenericItem::CheckKeys( attackdata_t *pAttData )
{
	...
	if( m_Hand == HAND_PLAYERHANDS && !fIsActiveItem ) return false;				//Player hands must be the active item in order to be used
	...
	{
		if( pAttData->ComboKeys[i][0] == '~' ) iThisCmd += 2;
		else iThisCmd++;
	}
	 ...
	 for (int i = 0; i < pAttData->ComboKeys.size(); i++) 
	{
		...
		int *TheMask;
		if( LastType == '+' || LastType == '~' ) TheMask = &Mask;
		else TheMask = &ClearMask;

		Mask = ClearMask = 0;
			
		if( CmdString == "attack1" )	SetBits( *TheMask, fIsActiveItem ? IN_ATTACK : IN_ATTACK2 );
		if( CmdString == "attack2" )	SetBits( *TheMask, IN_ATTACK2 ); //Thothie JUL2010_30
		if( CmdString == "forward" )	SetBits( *TheMask, IN_FORWARD );
		if( CmdString == "back" )		SetBits( *TheMask, IN_BACK );
		if( CmdString == "moveleft" )	SetBits( *TheMask, IN_MOVELEFT );
		if( CmdString == "moveright" )	SetBits( *TheMask, IN_MOVERIGHT );
		if( CmdString == "use" )		SetBits( *TheMask, IN_USE );
		if( CmdString == "jump" )		SetBits( *TheMask, IN_JUMP );
		...
		if( LastType == '-' ) 
		{
			if( FBitSet(KeyHistory[iThisCmd].Buttons,ClearMask) ) break;
		}
		else 
		{
			if( (KeyHistory[iThisCmd].Buttons ^ *TheMask) & *TheMask ) break;
		}
		...
		if( !fFirstKey )
		{
			...
			if( KeyHistory[iThisCmd].Time > TimeTooLate ) break;
		}
		...
		int IncAmt = 1;
		if( LastType == '~' && iThisCmd > 0 ) IncAmt = 2;
...
{
	if( !m_pOwner || !m_pOwner->IsAlive( ) ) return false;					//Owner must be alive
	if( m_Location != ITEMPOS_HANDS ) return false;							//Must be in owner's hands
	if( !m_Attacks.size() ) return false;									//Must have registered an attack
	if( !Spell_CanAttack( ) ) return false;									//Can't be in the process of preparing this spell
...
bool CGenericItem::StartAttack( int ForceAttackNum )
{
		...
		if( m_pPlayer && ForceAttackNum == -1 )
		{
			...
			if ( ClientAttacking )
			{
				...
				if( AttackNum == -1 ) //m_pPlayer->m_ClientAttackNum == -1 )
				{
					...
					if( CurrentAttack ) CurrentAttack->fAttackReleased = true;
					DoAttack = true;
				}
	...
	if( !m_pOwner || m_pOwner->IsShielding() //|| m_pOwner->IsActing( ) //Cant attack while acting (sitting, etc.)
		|| !m_pPlayer || FBitSet(m_pPlayer->m_StatusFlags, PLAYER_MOVE_NOATTACK) ) return false;
	...
	{
		dbg( "Force Attack" );
		if( ForceAttackNum >= (signed)m_Attacks.size() ) return true;
		...
		else if( !CurrentAttack )
		{
			 ...
			 for (int a = 0; a < m_Attacks.size(); a++) 
			{
				...
				if( m_TimeChargeStart || m_LastChargedAmt )
				{
					...
					if( !AttData.flChargeAmt ) continue;  //Don't do 'uncharged' while charging
				...
				if( CurrentAttack )
				{
					if( AttData.iPriority < CurrentAttack->iPriority ) continue;	//Selected attack has higher priority than this one
					if( AttData.iPriority == CurrentAttack->iPriority && !RANDOM_LONG(0,1) ) continue;	//Same priority - random chance of using this attack
				}				
	...
	if( iNewAttack >= 0 || iNewAttack == -2 )
	{
		...
		if( m_pOwner->Stamina < 0 ) m_pOwner->Stamina = 0;
		if( m_pPlayer ) m_pPlayer->AddNoise( CurrentAttack->flNoise );	//Make noise in world
		//Thothie FEB2008a
		...
		if( CurrentAttack->flChargeAmt ) m_TimeChargeStart = 0;
		...
		//Check for ammo
		if( !UseAmmo( CurrentAttack->iAmmoDrain ) ) CancelAttack( );
...
void CGenericItem::Attack( )
{
	...
	if( !m_pOwner || !CurrentAttack ) return;
	...
	{
		if( !m_pPlayer ) return; //Monsters MUST have fAttackThisFrame set to attack
		if( !fCanLandAttack || gpGlobals->time < CurrentAttack->tStart + CurrentAttack->tLandDelay ) return;
	}
	...
	else if( Type == ATT_CHARGE_THROW_PROJ ) ChargeThrowProj( );
...
void CGenericItem::RegisterAttack( )
{
	...
	if( attData.flChargeAmt > 0 ) attData.flChargeAmt = GET_CHARGE_FROM_TIME( attData.flChargeAmt );
	...
	else if( AttackType == "charge-throw-projectile" ) 
	{
		...
		if( HoldDelays.size() > 0 ) attData.tProjMinHold = atof( HoldDelays[0] );
		if( HoldDelays.size() > 1 ) attData.tMaxHold = atof( HoldDelays[1] );
	...
	//strike-land
	else attData.Type = ATT_STRIKE_LAND;
...
void CGenericItem::ItemPostFrame( )
{
	...
	if( m_Location == ITEMPOS_HANDS )	//Must be in hand to get activated
	{
		...
		if( FBitSet(m_pPlayer->pbs.ButtonsDown,iActivateButton) ) ActivateButtonDown( );	//Button is currently down
		else ActivateButtonUp( );														//Button is currently up
	}
	...
	#ifdef VALVE_DLL
		dbg( "Think" );
		if( MSProperties() & ITEM_GENERIC ) Think( );
	#endif
	dbg( "End" );
...
void CGenericItem::StrikeLand( )
{
	...
	if( m_pOwner->m_CurrentHand == LEFT_HAND ) SideMultiplier = -1;

	Vector vecSrc = m_pOwner->EyePosition( )
		+ gpGlobals->v_forward * CurrentAttack->StartOffset.y
		+ gpGlobals->v_right * CurrentAttack->StartOffset.x * SideMultiplier
		+ gpGlobals->v_up * CurrentAttack->StartOffset.z;
	...
	if ( m_pOwner->GetSkillStat( CurrentAttack->StatProf , CurrentAttack->PropProf ) < CurrentAttack->RequiredSkill ) thoth_unskilled = true;
	...
	if ( thoth_unskilled ) flHitPercentage = flHitPercentage * 0.25; //thothie - seems to work
	if ( m_pOwner->m_HITMulti > 0 ) flHitPercentage *= m_pOwner->m_HITMulti; //FEB2009_18
...
//	ALERT( at_console, "Use stat: %i  Value: %i %i %i\n", GetStatByName( (char*)STRING(CurrentAttack->sAttackStat) ), mSStat( CurrentAttack->sAttackStat, STATPROP_SPEED ), mSStat( CurrentAttack->sAttackStat, STATPROP_BALANCE ), mSStat( CurrentAttack->sAttackStat, STATPROP_POWER ) );
	...
	if ( thoth_unskilled ) flDamage *= 0.5; //this might be working
	...
	if( !m_pPlayer ) bitsDamage |= DMG_SIMPLEBBOX;
	if( CurrentAttack->flDamageAOERange ) SetBits( bitsDamage, DMG_AOE );
	if( CurrentAttack->NoDamage ) SetBits( bitsDamage, DMG_NONE );
...
bool CGenericItem::UseAmmo( int iAmt )
{
	startdbg;
	dbg( "Begin" );
	if( !m_pOwner || !CurrentAttack ) return false;
...
//Skip ammo check if the attack doesn't require ammo
	if( RequiresAmmo )
	{
		...
		if( !m_pPlayer ) 
		{
			...
			if( pGlobalItem ) CurrentAttack->sProjectile = pGlobalItem->ItemName;
		}
		else
		{
			...
			if ( m_pPlayer->m_ChosenArrow )
			{
					...
					(GENERIC || ((pArrow->iQuantity || iAmt > 1) && pArrow->iQuantity >= iAmt)) )
				{
					...
					{	
						...
						{
#ifdef VALVE_DLL
							if( pPack ) pPack->Container_RemoveItem( pArrow );
							else m_pOwner->RemoveItem( pArrow );
			...
			if ( !PreSelectedArrow )
			{
				...
				if( pProjInPack )
				{
					...
					//Remove the item after it has been depleted
						...
						{
							if( pPack ) pPack->Container_RemoveItem( pProjInPack );
							else m_pOwner->RemoveItem( pProjInPack );
...
void CGenericItem::ChargeThrowProj( )
{
	...
	if( pProjectile )
	{
		...
		Vector vStartPos = vOrigin + vForward * 10
			+ vForward * CurrentAttack->StartOffset.y
			+ vRight * CurrentAttack->StartOffset.x * (!m_Hand ? -1 : 1)
			+ vUp * CurrentAttack->StartOffset.z;
...
void DoDamage( damage_t &Damage, hitent_list &Hits )
{
	...
	else if( Damage.flRange > 0 )
	{
		...
		while ((pTarget = UTIL_FindEntityInSphere( pTarget, Damage.vecSrc, Damage.flRange )) != NULL)
		{
			...
			else						//AOE
			{
				...
				if( pHit ) Hits.add( hitent_t( dist, pHit ) );
			}
		}
	}
	...
	if( !Damage.flAOERange )
	{
		...
		else
		{
			...
			if( Damage.pAttacker ) SetBits( trflags, MSTRACE_HITCORPSES );
			...
			if( (Damage.outTraceResult.flFraction < 1.0f) && Damage.outTraceResult.pHit )
			{
				...
				if( pHit ) Hits.add( hitent_t( (Damage.vecEnd - Damage.vecSrc).Length() * Damage.outTraceResult.flFraction, pHit ) );
			}
			//BeamEffect( vecSrc, vecEnd, iBeam, 0, 0, 100, 10, 0, 255, 255, 255, 255, 20 );
		}
	}
	...
	if( pTarget )
	{
		...
		if( pAttMonster && Damage.AttackHit )
		{
			...
			if( pVictim && pVictim->IsAlive() )
			{
				...
				//Check if your horrid skill or bad luck made you miss:
				iAccuracyRoll = RANDOM_LONG( 0, 99 );
				if( iAccuracyRoll > Damage.flHitPercentage ) { Damage.AttackHit = false; Damage.AttackHit = false; }
			}
			else
			{
				// Always hit world entities or dead bodies
		...
		if( Damage.AttackHit ) //check again, because it might have been canceled in game_damaged_other
		{
			...
			if( pEntityInflictor ) pEntityInflictor->StoreEntity( pTarget, ENT_LASTSTRUCKBYME );
			if( pAttMonster )
			{
				...
				if( pItemInflictor )
				{
					...
					if( Damage.ItemCallBackPrefix ) CallbackName = *Damage.ItemCallBackPrefix + "_damaged_other";
				...
				if( pAttMonster->m_ReturnData.len( ) )
				{
					 ...
					 for (int i = 0; i < DamageRatios.size(); i++) 
					{
						if( DamageRatios[i] == "canceldamage" )
						{
							Damage.AttackHit = false; fReportHit = false; pTarget = NULL;
							goto EndDamage;  //Script canceled, skip the rest
						}
			...
			if( flDamage > 0 )  // flDamage < 0 means monster dodged it
			{
				...
				if ( pevAttacker ) ApplyMultiDamage( pevInflictor, pevAttacker ); //Thothie FEB2009 - experimenting
			}
		...
		if( pVictim ) pVictim->Attacked( pEntityAttacker, Damage );
		...
		if( fReportHit )
		{
			if ( Damage.flDamage > 0 )
			{
				...
				if ( dtype_code.starts_with("fire") ) strcpy(element_code," fire");
				else if ( dtype_code.starts_with("cold") ) strcpy(element_code," cold");
				else if ( dtype_code.starts_with("lightning") ) strcpy(element_code," lightning");
				else if ( dtype_code.starts_with("poison") ) strcpy(element_code," poison");
				else if ( dtype_code.starts_with("acid") ) strcpy(element_code," acid");
				else if ( dtype_code.starts_with("slash") ) strcpy(element_code," slash");
				else if ( dtype_code.starts_with("blunt") ) strcpy(element_code," blunt");
				else if ( dtype_code.starts_with("pierce") ) strcpy(element_code," pierce");
				else if ( dtype_code.starts_with("magic") ) strcpy(element_code," magic");
				else if ( dtype_code.starts_with("holy") ) strcpy(element_code," holy");
				else if ( dtype_code.starts_with("dark") ) strcpy(element_code," dark");
				else if ( dtype_code.starts_with("apostle") ) strcpy(element_code," apostle");
				else if ( dtype_code.starts_with("earth") ) strcpy(element_code," earth");
		...
		// Make a decal
		if ( !Damage.nodecal ) DecalGunshot( &Damage.outTraceResult, BULLET_PLAYER_CROWBAR );
...
EndDamage:
	...
	if( pAttMonster )
	{
		...
		if( pItemInflictor )
		{
			...
			if( Damage.ItemCallBackPrefix ) CallbackName = *Damage.ItemCallBackPrefix + "_dodamage";
