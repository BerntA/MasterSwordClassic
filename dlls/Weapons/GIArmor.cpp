...
#define TypeCheck if( !ArmorData ) return

void CGenericItem::RegisterArmor( )
{
	if( ArmorData ) delete ArmorData;
	...
	if( strstr(ProtectionArea,"head") )		ArmorData->m_ProtectionAreas.add( HBP_HEAD );
	if( strstr(ProtectionArea,"chest") )	ArmorData->m_ProtectionAreas.add( HBP_CHEST );
	if( strstr(ProtectionArea,"arms") )		ArmorData->m_ProtectionAreas.add( HBP_ARMS );
	if( strstr(ProtectionArea,"legs") )		ArmorData->m_ProtectionAreas.add( HBP_LEGS );

	msstring_ref ReplaceBodyParts = GetFirstScriptVar("ARMOR_REPLACE_BODYPARTS");
	if( strstr(ReplaceBodyParts,"head") )		m_WearModelPositions.add( HBP_HEAD );
	if( strstr(ReplaceBodyParts,"chest") )		m_WearModelPositions.add( HBP_CHEST );
	if( strstr(ReplaceBodyParts,"arms") )		m_WearModelPositions.add( HBP_ARMS );
	if( strstr(ReplaceBodyParts,"legs") )		m_WearModelPositions.add( HBP_LEGS );
...
{
	if( !ArmorData ) return Damage.flDamage;
