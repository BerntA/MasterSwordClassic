...
// Global GenericItem retrieve functions
	...
	CGenericItem *CGenericItemMgr::NewGenericItem( CGenericItem *pGlobalItem )
	{
		...
		dbg( "Create Entity for Item" );
			...
			pItem->pev = tmpPev;
			if( pItem ) pItem->m_pfnThink = NULL;
		#endif
	...
	void CGenericItemMgr::DeleteItem( CGenericItem *pItem )
	{
		 for (int i = 0; i < ItemCount(); i++) 
			if( m_Items[i].pItem == pItem ) DeleteItem( i );
	}
	...
	msstring_ref CGenericItemMgr::GetItemDisplayName( msstring_ref ItemName, bool Capital, bool Fullname, int Amt )
	{
		...
		if( pItem )
		{
			...
			else DisplayName = pItem->DisplayName( );
...
/*#ifdef VALVE_DLL
	...
	while( GetString(cString,(char *)pStringPtr,i,"\r\n") )
	{
		n = i;
		i += strlen(cString) + 1;
		if( n == i+1 ) continue; //Found nothing
		if( cString[0] == '/' && cString[1] == '/' ) continue; //comment
		...
		if( fSuccess ) NewItem.RunScriptEvents( );	//Slows down game load, but needed for the precachefile command
...
void CGenericItem::Deactivate( )
{
	...
	if( DrinkData )			{ delete (void *)DrinkData;			DrinkData = NULL;		}
	dbg( "Deallocate Armor data" );
	if( ArmorData )			{ delete (void *)ArmorData;			ArmorData = NULL;		}
	dbg( "Deallocate Projectile data" );
	if( ProjectileData )	{ delete (void *)ProjectileData;	ProjectileData = NULL;	}
	#ifdef VALVE_DLL
...
{
	if( m_pParentContainer ) return m_pParentContainer->Owner( );
	return m_pOwner;
}
...
bool CGenericItem::Deploy( )
{
	if( m_Location == ITEMPOS_HANDS )
	{
			...
			if( m_pPlayer ) 
			{
				...
				else strcpy( m_pPlayer->m_szAnimLegs, "" );
			}
		#endif
	...
	CallScriptEvent("game_deploy");
		...
		if( m_pPlayer ) m_pPlayer->m_ClientCurrentHand = -1;
...
void CGenericItem::ActivateButtonUp()
{
	...
	if( !m_ReleaseAttack )
		if( CurrentAttack && 
			...
			|| CurrentAttack->Type == ATT_STRIKE_HOLD) )
		{
...
void CGenericItem::FinishAttack( ) { //unused
/*	switch( iActionType ) {
...
void CGenericItem::ResetAttack( ) { //unused
		...
		case CE_SHIELDHIT:	//I hit a shield
			{
				...
				if( m_pOwner )
				{
					...
					{
						if( m_pPlayer ) m_pPlayer->SendInfoMsg( "Your %s breaks apart!\n", DisplayName() );
...
bool CGenericItem::ShouldIdle( void ) {
	if( IdleTime < gpGlobals->time && 
		fNextActionTime < gpGlobals->time ) return true;
	else return false;
}
#ifndef VALVE_DLL
void CGenericItem::Idle( void ) { } //add playing idle animations?
#endif
...
{
	if( m_NotUseable ) return false;

	CBasePlayer *pPlayer = NULL;
	bool fWentToPack = false;
	if( pReciever->IsPlayer() ) pPlayer = (CBasePlayer *)pReciever;

	//I'm an airborne projectile, cancel out
	if( MSProperties() & ITEM_PROJECTILE ) return false;
		...
		if( Return == -2 )
		{
			if( !fRemovedFromPack && AllowPutInPack )
			{
				...
				if( !pPlayer->PutInAnyPack( this, true ) )
					{ if( pPlayer->Gear.size() ) return false; }		//If there were no packs, show the hands error message below
				else  { fWentToPack = true; Return = 100; }
			}
		}
	...
	if( fSound ) EMIT_SOUND(ENT(pReciever->pev), CHAN_ITEM, "items/gunpickup2.wav", 1, ATTN_NORM);
		 ...
		 for (int iloc = 0; iloc < m_WearPositions.size(); iloc++) 
		{
			...
			if( iSlots > PlayerPos.MaxAmt )
			{
				if( Verbose )
					...
					{
						if( pItemConflict ) m_pPlayer->SendInfoMsg( "You have no more %s slots\n", PlayerPos.Name.c_str() ); //Thothie DEC2007a - was "You've are already wearing a %s\n", SPEECH_GetItemName(pItemConflict)
						else m_pPlayer->SendInfoMsg( "You can't wear %s\n", SPEECH_GetItemName(this) );
					}
					else m_pPlayer->SendInfoMsg( "You have no more %s slots\n", PlayerPos.Name.c_str() );
...
bool CGenericItem::WearItem( void )
{
	...
	if( IsActiveItem ) m_pOwner->SwitchToBestHand( );
...
CGenericItem *CGenericItem::FindPackForItem( CBasePlayer *pPlayer, bool fVerbose )
{
	...
	if( strstr(ItemName,"arrow") ) pPack = pPlayer->GetContainer( "quiver" );
	else if( strstr(ItemName,"swords") )  pPack = pPlayer->GetContainer( "sheath" );
	else if( strstr(ItemName,"blunt") ||
		     strstr(ItemName,"axes") )  pPack = pPlayer->GetContainer( "holster" );
	
	//Check for space, etc in desired pack
	if( pPack && CanPutInPack( pPack ) ) fSuccess = true;
...
bool CGenericItem::CanHolster( ) {
	if( m_Hand == HAND_PLAYERHANDS ) return true;
	if( CurrentAttack ) return false;
	return true;
}
...
void CGenericItem::FallThink( void )
{
	...
	else flNextThink = gpGlobals->time + 0.1;
}
...
void CGenericItem::RemoveFromOwner( ) 
{
	...
	if( m_pPlayer ) m_pPlayer->m_TimeResetLegs = 0;
	...
	if( m_pOwner ) m_pOwner->Gear.RemoveItem( this );	//Remove me from my owner's packlist
	...
	if( pev->iuser2 && pev->iuser3 ) //Notify torch sprite I've been removed
	{
		...
		if( (int)pSprite == pev->iuser3 ) pSprite->Think( );
	}
#endif
...
bool CGenericItem::Script_ExecuteCmd( CScript *Script, SCRIPT_EVENT &Event, scriptcmd_t &Cmd, msstringlist &Params )
{
	...
	msstring DebugString = msstring("Action:");
	 for (int i = 0; i < Cmd.m_Params.size(); i++) 
		{ DebugString += " "; DebugString += Cmd.m_Params[i]; }
...
//************************** WIPESPELLS by THOTHIE **********************
	...
	//****************************** REGISTERATTACK ************************
	else if( Cmd.Name() == "registerattack" ) RegisterAttack( );
	//****************************** REGISTERCONTAINTER ********************
	else if( Cmd.Name() == "registercontainer" ) RegisterContainer( );
	//****************************** REGISTERDRINK *************************
	else if( Cmd.Name() == "registerdrink" ) RegisterDrinkable( );
	//****************************** REGISTERARMOR *************************
	else if( Cmd.Name() == "registerarmor" ) RegisterArmor( );
	//****************************** REGISTERPROJECTILE ********************
	...
	//****************************** CANCELATTACK **************************
	else if( Cmd.Name() == "cancelattack" ) CancelAttack( );
	//****************************** SETSHIELD *****************************
	else if( Cmd.Name() == "setshield" ) {
		#ifdef VALVE_DLL
			if( Params.size( ) >= 1 )
				{ if( Params[0] != "none" ) ActivateShield( msstring("models/") + Params[0] ); }
	...
	//or player code if its a player
		...
		if( Params.size( ) >= 1 )
		{
			if( m_pOwner ) m_pOwner->CallScriptEvent( Params[0] );
		}
		else ERROR_MISSING_PARMS;
	}
	//****************************** VALUE **************************
	...
	//****************************** GROUPABLE **************************
		...
		if( Params.size( ) >= 1 )
		{
			...
			if( iMaxGroupable )
			{
				...
				if( iQuantity <= 0 ) iQuantity = 1; //FEB2010_13 MIB Stackable Stacks
				m_MaxGroupable = iMaxGroupable;
			}
			else ClearBits( Properties, ITEM_GROUPABLE );
		}
		else ERROR_MISSING_PARMS;
	}
	//****************************** USEABLE **************************
	...
	//****************************** SETANIMEXT **************************
		...
		{
			m_AnimExt = Params[0];
			if( Params.size() > 1 ) m_AnimExtLegs = Params[1];
		}
		else ERROR_MISSING_PARMS;
	}
	//***************************** SETVIEWMODEL *************************
	...
	//***************************** SETWORLDMODEL *************************
		...
		if( Params.size( ) >= 1 )
		{
			if( Params[0] == "none" ) WorldModel = "";
			else WorldModel = msstring("models/") + Params[0];
		}
		else ERROR_MISSING_PARMS;
	}
	//***************************** SETHOLDMODEL **************************
	...
	//********************* PLAYVIEWANIM **************************
			...
			if( Params.size( ) >= 1 )
			{
				if( m_pPlayer && m_Location == ITEMPOS_HANDS ) 
				{
					...
					else m_pPlayer->pev->weaponanim = iAnim;
	...
	else if( Cmd.Name() == "playowneranim" )
	{
		...
		if( Params.size( ) >= 1 )
		{
				...
				if( m_pOwner )
				{
					...
					if( m_pOwner->m_Scripts[0] ) m_pOwner->m_Scripts[0]->Script_ExecuteCmd( Event, Cmd, Params );
	...
	else if( Cmd.Name() == "svplaysound" || Cmd.Name() == "svplayrandomsound" || Cmd.Name() == "playsound" || Cmd.Name() == "playsoundcl" || Cmd.Name() == "playrandomsound" || Cmd.Name() == "playrandomsoundcl" )
	{
		...
		#endif
			{
				...
				if( m_pOwner && m_pOwner->m_Scripts[0] ) m_pOwner->m_Scripts[0]->Script_ExecuteCmd( Event, Cmd, Params );
				else return false;  //Fallback to the CScript version of playsound/playrandomsound
			}
	}
	//***************************** WIELD ***************************
			...
			if( m_pPlayer ) {
				byte ReqHands = atoi(Params[0]), UsedHands = 0;
				 for (int i = 0; i < MAX_PLAYER_HANDS; i++) 
					if( m_pPlayer->Hand(i) && m_pPlayer->Hand(i) != this ) UsedHands++;
	...
	else if( Cmd.Name() == "learnspell" )
	{
			...
			if( Params.size( ) >= 1 )
			{
				if( m_pPlayer )
				{
					...
					else CallScriptEvent( "game_learnspell_failed" );
				}
			}
			else ERROR_MISSING_PARMS;
		#endif
	...
	//****************************** SETDMG ******************************
		...
		if( Params.size() >= 2 )
		{
			if( m_CurrentDamage )
			{
				...
				if( DmgProp == "dmg" )			m_CurrentDamage->flDamage = atof(DmgValue);
				else if( DmgProp == "type" )	m_CurrentDamage->sDamageType = DmgValue;
				else if( DmgProp == "hit" )		m_CurrentDamage->AttackHit = atoi(DmgValue) ? true : false;
			}
		}
		else ERROR_MISSING_PARMS;
	}
...
CGenericItem *FindParryWeapon( CMSMonster *pMonster, /*out*/int &iPlayerHand, /*out*/int &iAttackNum )
{
	...
	iHand[0] = pMonster->m_CurrentHand; iHand[1] = !iHand[0];

	 for (int i = 0; i < MAX_PLAYER_HANDS; i++) 
	{
		int CheckHand = iHand[i];
		pHandItem = pMonster->Hand(CheckHand);
		if( !pHandItem ) continue;
...
CGenericItem *MSUtil_GetItemByID( ulong m_iId )
{
	...
	while( pItem )
	{
		if( pItem->m_iId == m_iId ) return pItem;
...
CGenericItem *MSUtil_GetItemByID( ulong m_iId, CMSMonster *pOwner )
{
	...
	if( !pItem || pItem->Owner() != pOwner ) return NULL;
...
CGenericItem *MSUtil_GetItemByID( ulong lID )
{
	 for (int e = 0; e < MSCLGlobals::m_ClEntites.size(); e++) 
	{
		...
		if( ((CGenericItem *)pEntity)->m_iId == lID ) return (CGenericItem *)pEntity;
	}
...
CGenericItem *ReadGenericItem( bool fAllowCreateNew )
{
   ...
   if( !pItem && fAllowCreateNew ) pItem = CGenericItemMgr::NewGenericItem( idx ); //ItemScript ); // MiB - See above
   if( !pItem )
   {
