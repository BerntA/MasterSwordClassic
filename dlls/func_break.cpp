...
{
	//JAN2013_08 Thothie - Scriptevent capability for func_break
...
void CBreakable::Precache( void )
{
	...
	case matComputer:
		...
		pGibName = "models/rockgibs.mdl"; ///Thothie - dont need this in MS - subsittuting with rockgibs (item limit)
...
// do the damage
	...
	if (pev->health <= 0)
	{
		...
		if ( m_scriptevent.len() )
		{
			...
			if (!FNullEnt( pevAttacker )) pOther = CBaseEntity::Instance( pevAttacker );
			if ( pOther->IsMSMonster() || pOther->IsPlayer() )
			{
				...
				} //end if ( m_scriptevent.starts_with("gm_") )
			} // end if ( pOther->IsMSMonster() || pOther->IsPlayer() )
		} //end if ( m_scriptevent.len() )
