...
{	
	...
	else logfile << "Client rejected: " << szRejectReason << endl;
...
void ClientCommand2( edict_t *pEntity )
{
	...
	if ( pPlayer->InMenu )  thoth_canuseinv = false; //SEP2011_07 - prevent inv use while in menus
	if ( FBitSet(pPlayer->m_StatusFlags, PLAYER_MOVE_NOATTACK) ) thoth_canuseinv = false; //can't use inventory cuz can't attack
	if ( FBitSet(pPlayer->m_StatusFlags, PLAYER_MOVE_SITTING) ) thoth_canuseinv = true; //but can't attack cuz sitting, so it's okay
	if ( !pPlayer->IsAlive() ) thoth_canuseinv = false; //JAN2010_19 wait, dead, can't use inventory after all
	if ( pPlayer->HasConditions( MONSTER_TRADING ) ) thoth_canuseinv = false; //SEP2011_10 - prevent inv use while in trade
	...
	else if ( FStrEq(pcmd, "char") && CMD_ARGV(1) ) //JAN2010_09 Thothie - Fixed sploit here where other character could be accessed instant - may also fix dbl click bug
	{
		...
		else if ( pPlayer->m_CharacterState == CHARSTATE_UNLOADED && pPlayer->m_CharacterState != CHARSTATE_LOADING )
		{
			...
			else if( CMD_ARGC() == 6 )									//Create character
			{
				...
				else ClientPrint( pPlayer->pev, at_console, "*** You can't create a new character on this map! ***\n" );
			}
		}
		else ClientPrint( pPlayer->pev, at_console, "*** Can't Access Character Menu - Character Loaded or Loading ***\n" );
	}
	...
	else if (FStrEq(pcmd, "+special")) pPlayer->pbs.MoreBTNSDown |= BTN_SPECIAL;
	else if (FStrEq(pcmd, "-special")) pPlayer->pbs.MoreBTNSDown &= ~BTN_SPECIAL;
	...
	else if ( FStrEq(pcmd, "get") && thoth_canuseinv ) pPlayer->GetAnyItems( );
	...
	else if( FStrEq(pcmd, "getmenuoptions") ) 
	{
		...
		if( CMD_ARGC() >= 2 )
		{
			...
			if( pEntity && pEntity->IsMSMonster() ) pMonster = (CMSMonster *)pEntity;
		}
		else
		{
			pMonster = pPlayer;
	...
	else if( FStrEq(pcmd, "menuoption") ) 
	{
		...
		int EntIdx = atoi( CMD_ARGV(1) );
		CBaseEntity *pEntity = MSInstance( INDEXENT(EntIdx) );
		if( !pEntity || !pEntity->IsMSMonster() ) return;
	...
	else if (FStrEq(pcmd, "trade")) 
	{
		...
		clrmem( tiTradeInfo );
		if( !stricmp(CMD_ARGV(1),"buy") )		{ tiTradeInfo.iStatus = TRADE_BUY; fDoTrade = true; }
		else if( !stricmp(CMD_ARGV(1),"sell") ) { tiTradeInfo.iStatus = TRADE_SELL; fDoTrade = true; }
					...
					if( pItem )
					{
						 ...
						 for (int ID = 0; ID < SHRT_MAX; ID++) 
						{
							...
							for (int i = 0; i < pStorage->Items.size(); i++)
								if (pStorage->Items[i].ID == ID) { Duplicate = true; break; }
				...
				{ 
					...
					if( pStorageItem )
					{
						...
						if( !Cost || pPlayer->m_Gold > Cost )			//If no retrieval fee or I have enough to pay it...
						{
							...
							else pItem->SUB_Remove( );
						}
						else pPlayer->SendEventMsg( HUDEVENT_UNABLE, msstring("You can't afford the retrieval fee of ") + Cost + " Gold" );
					}
				}
	...
	else if( FStrEq(pcmd, "reset_quests") ) 
	{
		...
		{
			if ( FBitSet(pPlayer->m_StatusFlags, PLAYER_MOVE_NOMOVE) ) pPlayer->SendInfoMsg("Can't prepare spells while frozen or stunned...");
			else pPlayer->SendInfoMsg("Can't prepare spells now.");
		}
	}
	...
	else if ( FStrEq(pcmd, "drop" ) )
	{
		...
		if ( thoth_canuseinv )
		{
			...
			if( CMD_ARGC() > 1 ) pItem = MSUtil_GetItemByID( atol(CMD_ARGV(1)), pPlayer );	
			else pItem = pPlayer->ActiveItem();

			UTIL_MakeVectors( pev->angles );
			if( pItem ) pPlayer->DropItem( pItem, false, true );
			else MSErrorConsoleText( "ClientCommand()", msstring("'drop' cmd couldn't find item to drop") );
		}
		else
		{
			if ( FBitSet(pPlayer->m_StatusFlags, PLAYER_MOVE_NOMOVE) ) pPlayer->SendInfoMsg("Can't use inventory while frozen or stunned...");
...
*/
	...
	else if ( FStrEq(pcmd, "use") )
	{
		...
		if ( thoth_canuseinv )
		{
			...
			if( CMD_ARGC() > 1 ) Hand = atoi( CMD_ARGV(1) );	
	...
	else if (FStrEq(pcmd, "action")) { //'act' is a reserved Half-life command
		if( !CMD_ARGV(1) ) return;
...
/*	else if (FStrEq(pcmd, "effect") && g_flWeaponCheat) {
	...
	{
		if ( pPlayer->pev->weaponanim > 0 ) return;
		...
		else if( FStrEq( CMD_ARGV(1), "get" ) ) 
		{
			...
			if( pContainer )
			{
				...
				else MSErrorConsoleText( "ClientCommand - inv get item", msstring("Item ") + atoi( CMD_ARGV(3) ) + " does not exist on player!" );
			}
			else MSErrorConsoleText( "ClientCommand - inv get item", msstring("Pack ") + atoi( CMD_ARGV(2) ) + " does not exist on player!" );
		}
	...
	/*	else if ( FStrEq(pcmd, "dotrick") )
		...
		else if ( FStrEq(pcmd, "kick" ) ) { // allow kicking
			if( CMD_ARGV(1) )
				SERVER_COMMAND( UTIL_VarArgs("kick %s\n", CMD_ARGV(1)) );
		}
		else if ( FStrEq(pcmd, "give" ) ) { // free items
			if( CMD_ARGC() >= 2 )
			{
				CBaseEntity *pEnt = pPlayer->GiveNamedItem( CMD_ARGV(1) );
				if( CMD_ARGC() >= 3 && pEnt && (pEnt->MSProperties() & ITEM_GENERIC) ) ((CGenericItem *)pEnt)->iQuantity = atoi(CMD_ARGV(2));
			}
		}
		else if ( FStrEq(pcmd, "setstat" ) ) { // free stats
			if( CMD_ARGV(3) )
			{
		...
		else if ( FStrEq(pcmd, "sethp" ) ) { // set hp
			if( CMD_ARGV(1) )
			{
		...
		else if ( FStrEq(pcmd, "setmp" ) ) { // set mp
			if( CMD_ARGV(1) )
			{
				pPlayer->m_MP = 0;
		...
		*/
		else if ( FStrEq(pcmd, "iamgod" ) ) { // god mode
			if( CMD_ARGV(1) ) {
				if( atoi(CMD_ARGV(1)) )
					SetBits( pPlayer->pev->flags, FL_GODMODE );
				else ClearBits( pPlayer->pev->flags, FL_GODMODE );
			}
		}
		else if (FStrEq(pcmd, "freegold") ) pPlayer->GiveGold( 1000 );
...
const char *GetGameDescription()
{
	...
	msstring CentralSVTag = (MSCentral::Enabled() &&
							!MSCentral::m_Online) ? CVAR_GET_STRING("ms_central_tag") : "";
...
int AddToFullPack( struct entity_state_s *state, int e, edict_t *ent, edict_t *host, int hostflags, int player, unsigned char *pSet )
{
	...
	state->aiment = 0;
	if ( ent->v.aiment ) state->aiment = ENTINDEX( ent->v.aiment );
	...
	if( FBitSet( ent->v.playerclass, ENT_EFFECT_FOLLOW_ROTATE ) )	//For Master Sword's special follow
	{
		...
		if( ent->v.owner ) state->owner = ENTINDEX( ent->v.owner );	
	}
