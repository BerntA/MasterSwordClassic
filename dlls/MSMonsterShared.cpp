...
bool CMSMonster::IsActing( ) 
{
	bool Charging = false;
	 for (int i = 0; i < Gear.size(); i++) 
		if( Gear[i]->Attack_IsCharging( ) ) { Charging = true; break; }
...
bool CMSMonster::GetItem( getitem_t &ItemDesc )
{
	 ...
	 for (int t = 0; t < 2; t++) 
	{
		...
		if( (ItemDesc.IgnoreHands && Loop_CheckHands)
		||  (ItemDesc.IgnoreWornItems && !Loop_CheckHands) )
			continue;
		 ...
		 for (int i = 0; i < Gear.size(); i++) 
		{
			...
			if( ( ItemDesc.CheckPartialName && pItem->ItemName.m_string.contains(ItemDesc.Name))
			||  (!ItemDesc.CheckPartialName && pItem->ItemName == ItemDesc.Name) )
			{
			 ...
			 for (int n = 0; n < pItem->Container_ItemCount(); n++) 
			{
				...
				if( ( ItemDesc.CheckPartialName && pItem->ItemName.m_string.contains(ItemDesc.Name))
					||  (!ItemDesc.CheckPartialName && pItemInPack->ItemName == ItemDesc.Name) )
				{
					ItemDesc.retFound = true;
...
CGenericItem *CMSMonster::GetItemInInventory( uint StartID, bool WeaponOnly, bool CheckHands, bool CheckWorn )
{
	 ...
	 for (int i = 0; i < Gear.size(); i++) 
	{
		...
		if( !FBitSet(pPack->MSProperties(),ITEM_CONTAINER) )
		 ...
		 for (int n = 0; n < pPack->Container_ItemCount(); n++) 
			Items.add( pPack->Container_GetItem( n ) );
	}
...
bool CMSMonster::AddItem( CGenericItem *pItem, bool ToHand, bool CheckWeight, int ForceHand )
{
	...
	if( ToHand )
	{
		...
		if( iAddHand < 0 )
		{
			...
			if( iAddHand < 0 ) return false;
		}

		pItem->m_Location = ITEMPOS_HANDS;
		pItem->m_Hand = iAddHand;
	}
	else if( !CanHold( pItem ) ) return false;
	...
	if( IsPlayer() && iStatType == 0 ) 
	{
		...
		case NATURAL_STR:
			return (GetSkillStat(SKILL_SWORDSMANSHIP) * 1.5 +
					...
					(GetSkillStat(SKILL_ARCHERY) * 0.7)) / 4;
		...
		case NATURAL_CON:
			return (GetSkillStat(SKILL_ARCHERY) +
					GetSkillStat(SKILL_PARRY) +
					GetSkillStat(SKILL_SPELLCASTING)) / 2;	
		case NATURAL_AWR:
			//Archery & parry count the most for awareness
			return (GetSkillStat(SKILL_SWORDSMANSHIP) * 0.5f +
					GetSkillStat(SKILL_MARTIALARTS) +
				    GetSkillStat(SKILL_AXEHANDLING) * 0.5f +
					GetSkillStat(SKILL_BLUNTARMS) * 0.5f +
					GetSkillStat(SKILL_SMALLARMS) +
		...
		case NATURAL_FIT:
			...
			return (GetSkillStat(SKILL_SWORDSMANSHIP) +
					GetSkillStat(SKILL_MARTIALARTS) +
				    GetSkillStat(SKILL_AXEHANDLING) +
					GetSkillStat(SKILL_BLUNTARMS) +
				    GetSkillStat(SKILL_PARRY) + 
					(GetSkillStat(SKILL_SMALLARMS) * 0.8) +
					(GetSkillStat(SKILL_ARCHERY) * 0.6)) / 5;
		case NATURAL_WIS:
			return (GetSkillStat(SKILL_PARRY) +
					GetSkillStat(SKILL_SPELLCASTING)) / 2;	
		}
		return 0;
	}
