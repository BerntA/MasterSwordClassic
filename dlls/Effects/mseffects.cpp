...
void CTorchLight :: Think( )
{
	...
	if( pTorchOwner ) UTIL_MakeVectors( pTorchOwner->pev->angles );

	if( FBitSet( pTorchTouse->pev->effects, EF_NODRAW ) ||
		pTorchTouse->pev->waterlevel > 2 ) 
		for( int i = 0; i < TORCH_LIGHTS; i++ ) SetBits( pLight[i]->pev->effects, EF_NODRAW );
	else for( int i = 0; i < TORCH_LIGHTS; i++ ) ClearBits( pLight[i]->pev->effects, EF_NODRAW );
...
{
	for( int i = 0; i < TORCH_LIGHTS; i++ ) pLight[i]->SUB_Remove( );
...
public:
	...
	void Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
	{
		...
		else if( pCaller && pCaller->IsPlayer() ) pPlayer = (CBasePlayer *)pCaller;
...
void CChangePlayerSpeed :: Think( void )
{
	...
	for( int i = 0; i < 256; i++ )
	{
		...
		if( gpGlobals->time < pInfo->TimeResetSpeed ) continue;
...
void CChangePlayerSpeed :: Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if( m_Duration >= 0 ) pInfo->TimeResetSpeed = gpGlobals->time + m_Duration;
	else pInfo->TimeResetSpeed = -1;
}
...
#define REQPARAMS( a ) if( Params.size() < a ) { ALERT( at_console, "Script: %s, effect '%s' missing parameters!\n", m.ScriptFile.c_str(), Params[0].c_str() ); return; }
...
void CScript::ScriptedEffect( msstringlist &Params )
{
	...
	if( Params[0] == "beam" )
	{
			...
			if ( pBeam )
			{
				...
				else if ( Params[3] == "end_target" )
				{
					...
					if ( pTargetNew )
					{
						...
						if ( Params.size() >= 6 ) pBeam->SetEndAttachment( atoi(Params[5]) );
					}
				}
				else if ( Params[3] == "start_target" )
				{
					...
					if ( pTargetNew )
					{
						...
						if ( Params.size() >= 6 ) pBeam->SetStartAttachment( atoi(Params[5]) );
					}
				}
				...
				else if ( Params[3] == "remove" )
				{
					...
					if ( rem_delay == 0 ) pBeam->pev->nextthink = gpGlobals->time + 0.1;
					else pBeam->pev->nextthink = gpGlobals->time + rem_delay;
				}
			}
		...
		else if( Params[1] == "follow" )
		{
			...
			MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY );
				...
				//h4x
				if ( Params[4] == "0" ) WRITE_SHORT( pTarget->entindex() | 0x0000 );
				else if ( Params[4] == "1" ) WRITE_SHORT( pTarget->entindex() | 0x1000 );
				else if ( Params[4] == "2" ) WRITE_SHORT( pTarget->entindex() | 0x2000 );
				else if ( Params[4] == "3" ) WRITE_SHORT( pTarget->entindex() | 0x3000 );
				else if ( Params[4] == "4" ) WRITE_SHORT( pTarget->entindex() | 0x4000 );
		...
		if( Type != 1 && !m.pScriptedEnt ) return; //'end' and 'ents' need an owner for entity lookups.  'points' doesn't need one 
		...
		if (remove_delay > 0)	Beam->SetThink(&CBaseEntity::SUB_Remove);
	...
	else if( Params[0] == "screenfade" )
	{
		...
		if( ApplyToAll || (pEntity && pEntity->IsPlayer( )) )
		{
			...
			if( Params[6].find("fadein") != msstring_error )	Flags |= FFADE_IN;
			if( Params[6].find("fadeout") != msstring_error )	Flags |= FFADE_OUT;
			if( Params[6].find("noblend") != msstring_error )	Flags |= FFADE_MODULATE;
			if( Params[6].find("perm") != msstring_error )		Flags |= FFADE_STAYOUT;
	...
	else if( Params[0] == "glow" )
	{
		...
		if( Duration < 0 ) pGlowEnt->SUB_Remove( ); //Glow forever
	}
