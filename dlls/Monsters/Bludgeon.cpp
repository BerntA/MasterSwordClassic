...
void CBludgeon::Think()
{
	...
	if (m_hEnemy == NULL || !m_hEnemy->IsAlive() || m_hEnemy->pev->flags & FL_NOTARGET || gpGlobals->time > FindTargetTime)
	{
		...
		if (m_hEnemy != NULL)
		{
			...
			if (ehPrevEnemy != NULL && ehPrevEnemy.Get() != m_hEnemy.Get())
				m_LastEnemy = ehPrevEnemy;
		...
		else if (m_LastEnemy != NULL)
		{
			...
			if (m_LastEnemy->IsAlive() && FVisible(m_LastEnemy))
				m_hEnemy = m_LastEnemy;
			FindTargetTime = gpGlobals->time + INTERVAL_SWITCHTARGET1;
		}
		else if (ehPrevEnemy != NULL)
		{
			//			ALERT( at_console, "ehPrevEnemy\n" );
			if (ehPrevEnemy->IsAlive())
				m_hEnemy = ehPrevEnemy;
			FindTargetTime = gpGlobals->time + INTERVAL_SWITCHTARGET1;
		}
		if (m_hEnemy == NULL)
		{	//m_hEnemy is still invalid
			//			ALERT( at_console, "m_hEnemy == NULL\n" );
			//StopWalking( );
	...
	if (gpGlobals->time >= AttackTime)
		Attack();
	if (gpGlobals->time >= AttackLandTime)
		AttackLand();
	if (bAttacking)
		FinishAttack();
	if (gpGlobals->time >= MoveTime)
		Move();
...
{
	if (AttackLandTime || m_Activity == ACT_HOP)
		return;
	if (m_hEnemy == NULL || !m_hEnemy->IsAlive())
		return;
	Vector m_vecTarget = m_hEnemy->EyePosition() - EyePosition();
	//Are we close enough?
	if (m_vecTarget.Length() > AttackProximity)
		return;
	StopWalking();
...
void CBludgeon::Move()
{
	if (bAttacking || m_Activity == ACT_HOP)
		return;
	bCanChangeVel = TRUE;
	...
	if (m_hEnemy != NULL && m_hEnemy->IsAlive())
	{
		...
		else if (m_vecEnemyLKP)
			m_vecTarget = m_vecEnemyLKP - EyePosition();
...
*/
		...
		CMSMonster::Move(); //No enemy, walk around randomly (here m_vecEnemyLKP is the random place to walk)
...
{
	if (pTarget->pev->classname == pev->classname)
		return R_AL;
