...
{
	if( !pszSequence || !strlen(pszSequence) ) return false;
...
//		if( bCustomLegs ) NewGait = 0;
	...
	else m_pPlayer->pev->gaitsequence = NewGait;
}
...
void CAnimHold :: Initialize( void *vData ) {
	ReleaseAnim = TRUE; UseGait = false; //defaults
	int Flags = (int)vData;
	if( Flags & (1<<0) ) ReleaseAnim = FALSE;	//Release to any anim besides walk
	if( Flags & (1<<1) ) UseGait = TRUE;		//Use gait
	IsNewAnim = TRUE;
}
...
void CAnimHold :: Animate( ) {
	...
	if( IsNewAnim ) SetAnim( m_pOwner->pszCurrentAnimName );

	if( UseGait ) GaitAnimate( );
	else m_pOwner->pev->gaitsequence = 0;
}
