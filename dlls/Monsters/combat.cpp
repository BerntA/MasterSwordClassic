...
void CGib :: SpawnHeadGib( entvars_t *pevVictim )
{
	...
	if ( g_Language == LANGUAGE_GERMAN )
	{
		pGib->Spawn( "models/hgibs.mdl" );// throw one head - thothie germangibs.mdl doesnt exit
		pGib->pev->body = 0;
	}
	else
	{
...
void CGib :: SpawnRandomGibs( entvars_t *pevVictim, int cGibs, int human )
{
	...
	for ( cSplat = 0 ; cSplat < cGibs ; cSplat++ )
	{
		...
		if ( g_Language == LANGUAGE_GERMAN )
		{
			pGib->Spawn( "models/hgibs.mdl" ); //thothie germangib model doesnt exist
...
void CBaseMonster :: GibMonster( void )
{
	...
	if ( HasHumanGibs() )
	{
		return; //MAR2008b f*ck it - just wanna see what happens if we abort all monster gibs
...
Activity CBaseMonster :: GetDeathActivity ( void )
{
	...
	//Thothie DEC2010_02 - restoring death activity lookup
	/*
...
int CBaseMonster :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	//Master Sword - Players die when healh is < 1
   // if( IsPlayer() && pev->health > 0 && pev->health < 1 ) pev->health = 0;
	...
	while ((pEntity = UTIL_FindEntityInSphere( pEntity, vecSrc, flRadius )) != NULL)
	{
		if ( pEntity->pev->takedamage != DAMAGE_NO )
		{
			...
			{// the explosion can 'see' this entity, so hurt them!
				...
				{
					ClearMultiDamage( );
					pEntity->TraceAttack( pevInflictor, pevAttacker, flAdjustedDamage, (tr.vecEndPos - vecSrc).Normalize( ), &tr, bitsDamageType );
...
//=========================================================
bool CBaseEntity :: FInViewCone ( CBaseEntity *pEntity, float m_flFieldOfView )
{
	Vector2D	vec2LOS;
	float	flDot;
...
bool CBaseMonster :: FInViewCone ( CBaseEntity *pEntity ) {
	return CBaseEntity::FInViewCone( pEntity, m_flFieldOfView );
}
...
// in 2d, making the view cone infinitely tall. 
//=========================================================
bool CBaseEntity :: FInViewCone ( Vector &Origin, float m_flFieldOfView )
{
	Vector2D	vec2LOS;
	float		flDot;
...
bool CBaseMonster :: FInViewCone ( Vector &Origin ) {
	return CBaseEntity::FInViewCone( Origin, m_flFieldOfView );
}
//=========================================================
// FMVisible - returns true if a line can be traced from
// the caller's eyes to the target (w/ monsters)
//=========================================================
BOOL CBaseEntity :: FMVisible ( CBaseEntity *pEntity )
{
	TraceResult tr;
	...
	*/
	...
	UTIL_TraceLine(vecLookerOrigin, vecTargetOrigin, dont_ignore_monsters, dont_ignore_glass, ENT(pev)/*pentIgnore*/, &tr);
	
	if (tr.flFraction != 1.0)
	{
		if( tr.pHit == pEntity->edict() ) return TRUE; //hit my target
...
//=========================================================
// FMVisible - returns true if a line can be traced from
// the caller's eyes to the target vector (w/ monsters)
//=========================================================
BOOL CBaseEntity :: FMVisible ( const Vector &vecOrigin )
{
	TraceResult tr;
	Vector		vecLookerOrigin;
	...
	UTIL_TraceLine(vecLookerOrigin, vecOrigin, dont_ignore_monsters, dont_ignore_glass, ENT(pev)/*pentIgnore*/, &tr);
...
================
*/
float CBaseEntity::TraceAttack(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType, int iAccuracyRoll )
{
	Vector vecOrigin = ptr->vecEndPos - vecDir * 4;

	//Master Sword - make a sound based on the weapon used
	//StruckSound( CBaseEntity::Instance( pevInflictor ), CBaseEntity::Instance( pevAttacker ), flDamage, ptr, bitsDamageType );
...
void CBaseEntity::FireBullets(ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker )
{
	...
	for (ULONG iShot = 1; iShot <= cShots; iShot++)
	{
		...
		if (tr.flFraction != 1.0)
		{
			...
			if ( iDamage )
			{
				pEntity->TraceAttack(pev, pevAttacker, iDamage, vecDir, &tr, DMG_BULLET | ((iDamage > 16) ? DMG_ALWAYSGIB : DMG_NEVERGIB) );
			...
			case BULLET_PLAYER_9MM:		
				pEntity->TraceAttack(pev, pevAttacker, gSkillData.plrDmg9MM, vecDir, &tr, DMG_BULLET); 
				break;

			case BULLET_PLAYER_MP5:		
				pEntity->TraceAttack(pev, pevAttacker, gSkillData.plrDmgMP5, vecDir, &tr, DMG_BULLET); 
				break;

			case BULLET_PLAYER_BUCKSHOT:	
				 // make distance based!
				pEntity->TraceAttack(pev, pevAttacker, gSkillData.plrDmgBuckshot, vecDir, &tr, DMG_BULLET); 
				break;
			
			case BULLET_PLAYER_357:		
				pEntity->TraceAttack(pev, pevAttacker, gSkillData.plrDmg357, vecDir, &tr, DMG_BULLET); 
				break;

			case BULLET_MONSTER_9MM:
				pEntity->TraceAttack(pev, pevAttacker, gSkillData.monDmg9MM, vecDir, &tr, DMG_BULLET);
			...
			case BULLET_MONSTER_MP5:
				pEntity->TraceAttack(pev, pevAttacker, gSkillData.monDmgMP5, vecDir, &tr, DMG_BULLET);
			...
			case BULLET_MONSTER_12MM:		
				pEntity->TraceAttack(pev, pevAttacker, gSkillData.monDmg12MM, vecDir, &tr, DMG_BULLET); 
				if ( !tracer )
				{
					TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
					DecalGunshot( &tr, iBulletType );
			...
			case BULLET_NONE: // FIX 
				pEntity->TraceAttack(pev, pevAttacker, 50, vecDir, &tr, DMG_CLUB);
				TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
				// only decal glass
