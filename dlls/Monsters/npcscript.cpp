...
bool CMSMonster::Script_ExecuteCmd( CScript *Script, SCRIPT_EVENT &Event, scriptcmd_t &Cmd, msstringlist &Params )
{
	...
	{	//say <sound>[mouth open length]
		if( Params.size() >= 1 )
		{
			 for (int i = 0; i < Params.size(); i++) 
			{
				...
				{
					if( lengthpos ) UseWord = FullWord.substr( 0, lengthpos );
	...
	//hp <current> <max>
		...
		if( Params.size() < 1 ) ERROR_MISSING_PARMS;
	}
	//************************** WIDTH **************************
	...
	//************************** BLOOD *************************
		...
		if( Params.size() >= 1 )
		{
			if( Params[0] == "red" ) m_bloodColor = BLOOD_COLOR_RED;
			else if( Params[0] == "green" ) m_bloodColor = BLOOD_COLOR_GREEN;
			else if( Params[0] == "none" ) m_bloodColor = DONT_BLEED;
			else MSErrorConsoleText( "CGenericItem::ExecuteScriptCmd", UTIL_VarArgs("Script: %s, %s - invalid blood color!\n", Script->m.ScriptFile.c_str(), Cmd.Name().c_str()) );
		}
		else ERROR_MISSING_PARMS;
	}
	//************************** FOV ***************************
	...
	*/
	...
	else if( Cmd.Name() == "nopush" )
	{
		if( Params.size() >= 1 )
		{
			...
			if( atoi(Params[0]) ) m_nopush = true;
			else m_nopush = false;
		}
		else ERROR_MISSING_PARMS;
	}
	...
	//************************** ROAM **************************
		...
		if( Params.size() >= 1 )
		{
			...
			else ClearConditions( MONSTER_ROAM );
		}
		else ERROR_MISSING_PARMS;
	}
	//************************** BLIND **************************
	//MAR2008b
		...
		if( Params.size() >= 1 )
		{
			...
			if( atoi(Params[0]) ) SetConditions( MONSTER_BLIND );
			else ClearConditions( MONSTER_BLIND );
		}
		else ERROR_MISSING_PARMS;
	}
	//************************** INVISIBLE **************************
	...
	else if( Cmd.Name() == "expadj" )
	{
		if( Params.size() >= 1 )
		{
			...
			else MSErrorConsoleText( "CGenericItem::ExecuteScriptCmd", UTIL_VarArgs("Script: %s, %s - initial XP not set, ignoring\n", Script->m.ScriptFile.c_str(), Cmd.Name().c_str()) );
		}
		else ERROR_MISSING_PARMS;
	}
	//************************** FLOAT *************************
		...
		if( Params.size() >= 1 )
		{
			if( atoi(Params[0]) ) SetBits( pev->flags, FL_FLOAT|FL_SWIM );
			else ClearBits( pev->flags, FL_FLOAT|FL_SWIM );
		}
		else ERROR_MISSING_PARMS;
	}
	//************************* FLY ****************************
		...
		if( Params.size() >= 1 )
		{
			if( atoi(Params[0]) ) SetBits( pev->flags, FL_FLY );
			else ClearBits( pev->flags, FL_FLY );
	...
	//************************** OFFER **********************
		...
		if( Params.size() >= 2 )
		{
			...
			if ( pPlayer )
			{
				if ( pPlayer->NumItems() >= THOTH_MAX_ITEMS ) inventory_full = true;
			}
			...
			if ( !inventory_full )
			{
				...
				if( pEnt && pEnt->MyMonsterPointer() )
				{
					...
					if( ItemType == ITEM_NORMAL )
					{
						...
						else ALERT( at_console, "%s - offer: item %s not found!", DisplayName(), Params[1] );
					}
	...
	//************************** NPCSTORE.CREATE / CREATESTORE **************************
		...
		if( Params.size() >= 1 )
		{
			...
			if( !NewStore ) NewStore = CStore::m_gStores.add( new CStore );
	...
	//************************** NPCSTORE.ADDITEM / ADDSTOREITEM ***********************
		...
		if( Params.size() >= 2 )
		{
			...
			*/
			...
			if( Params.size() >= 3 ) iQuantity = atoi(Params[2]);  
			if( Params.size() >= 4 ) iCost = atoi(Params[3]);
			if( Params.size() >= 5 ) flSellRatio = min( atof(Params[4]), .9 );
			if( Params.size() >= 6 ) iBundleAmt = atoi(Params[5]);
			...
			if( NewStore ) 
			{
				...
				else MSErrorConsoleText( "CMSMonster::Script_ExecuteCmd()", UTIL_VarArgs("Script: %s, %s: non-existant item %s!\n", Script->m.ScriptFile.c_str(), Cmd.Name().c_str(), ItemName) );
			}
	...
	//************************** NPCSTORE.OFFER / OFFERSTORE **************************
		...
		if( Params.size() >= 3 )
		{
			...
			int iBuyFlags = 0;
			if( strstr(BuyFlags,"buy") ) SetBits( iBuyFlags, STORE_BUY );
			if( strstr(BuyFlags,"sell") ) SetBits( iBuyFlags, STORE_SELL );
			if( strstr(BuyFlags,"inv") ) iBuyFlags = STORE_INV;
			...
			if( !HasConditions( MONSTER_TRADING ) )
			{
				...
				if( NewStore && !inventory_full ) {
					...
					if( m_TradeCallBackEvent.len() ) CallScriptEvent( UTIL_VarArgs( "%s_success", m_TradeCallBackEvent.c_str() ) );
				}
				else if( m_TradeCallBackEvent.len() ) CallScriptEvent( UTIL_VarArgs( "%s_fail", m_TradeCallBackEvent.c_str()  ) );
			}
			else if( m_TradeCallBackEvent.len() ) CallScriptEvent( UTIL_VarArgs( "%s_busy", m_TradeCallBackEvent.c_str()  ) );
		}
		else ERROR_MISSING_PARMS;
	}
	//************************** NPCSTORE.REMOVE **************************
	...
	else if( Cmd.Name() == "menuitem.register" ) 
	{
		if( m_MenuCurrentOptions )
		{
			...
			if( Type == "callback" ) MenuOption.Type = MOT_CALLBACK;
			else if( Type == "say" ) MenuOption.Type = MOT_SAY;
			else if( Type == "payment" ) MenuOption.Type = MOT_PAYMENT;
			else if( Type == "payment_silent" )
			{
				MenuOption.Type = MOT_PAYMENT;
				MenuOption.SilentPayment = true;
			}
			else if( Type == "disabled" ) MenuOption.Type = MOT_DISABLED;
			else if( Type == "itemdesc" ) MenuOption.Type = MOT_DESC;
			else if( Type == "forgive" ) MenuOption.Type = MOT_FORGIVE;
			else if( Type == "green" ) MenuOption.Type = MOT_GREEN; //Thothie AUG2013_12 - new green type for completed tally
			else MenuOption.Type = MOT_CALLBACK;
	...
	//******************************* TAKEDMG ****************************
		...
		if( Params.size() >= 2 )
		{
			...
			else
			{
				...
				if ( !found_entry ) m.TakeDamageModifiers.add( TDM );
				msstring Debug_AMT = Params[1];
				msstring Debug_Type = Params[0];
				msstring thoth_var = "IMMUNE_";
				if ( Debug_Type.starts_with("cold") ) thoth_var.append("COLD");
				if ( Debug_Type.starts_with("fire") ) thoth_var.append("FIRE");
				if ( Debug_Type.starts_with("lightning") ) thoth_var.append("LIGHTNING");
				if ( Debug_Type.starts_with("poison") ) thoth_var.append("POISON");
				if ( Debug_Type.starts_with("holy") ) thoth_var.append("HOLY");
				if ( Debug_Type.starts_with("acid") ) thoth_var.append("ACID");
				if ( Debug_Type.starts_with("magic") ) thoth_var.append("MAGIC");
				float thoth_final_val = flModifier;
				if ( flModifier == 0 ) SetScriptVar(thoth_var,"1");
				if ( flModifier == 1 ) SetScriptVar(thoth_var,"0.99");
				if ( thoth_final_val != 1 && thoth_final_val > 0  ) SetScriptVar(thoth_var,flModifier);
			}
		}
		else ERROR_MISSING_PARMS;
	}
	//******************************** DODAMAGE ***************************
		...
		if( Params.size() >= 4 )
		{
			...
			{
				if( Params[0][0] != '(' ) Damage.sDamageType = Params[4];
			}
			...
			if( Params[1] == "direct" ) 
			{
				...
				{
					if ( !Damage.pDirectDmgTarget->IsAlive() ) return 0;
				}
				if ( Params.size() >= 6 ) Damage.sDamageType = Params[5]; //Thothie
			}
			else
			{
			...
			if( Params[0][0] == '(' )
			{
				...
				if( Params.size() >= 5 ) Damage.flAOEAttn = atof(Params[4]);
				if( Params.size() >= 6 )
				{
					if( Params[5].contains( "reflective" ) ) SetBits( Damage.iDamageType, DMG_REFLECTIVE );
				}
				if ( Params.size() >= 7 ) Damage.sDamageType = Params[6];  //Thothie
	...
	//************************** TOSSPROJECTILE ***************************
		...
		if( Params.size() >= 4 )
		{
			...
			//Thothie SEP2007a - make dmg multipliers internal
			if ( m_DMGMulti > 0 ) flDamage *= m_DMGMulti;
			...
			if( fAttack )
			{
				...
				if( pProjectile )
				{
					...
					if( Params.size() >= 6 )
					{
						if( Params.size() >= 7 )
						{
							if ( Params[6] == "notoffset" ) vStartPos = StringToVec( Params[5] );
						}
						else
						{
			...
			if ( IsPlayer() )
			{
				if ( !thoth_instat.contains(".") )
				{
					...
					if( pStat )
					{
						...
						{
							if( (signed)Params.size() <= NextParm ) break;			//Keep assigning stats until I run out of stats (for loop) or out of parameters (this check)
							pStat->m_SubStats[i].Value = atoi(Params[NextParm++]);
						}
					}
					else MSErrorConsoleText( "CMSMonster::Script_ExecuteCmd()", UTIL_VarArgs("Script: %s, %s: stat %s not found!\n", Script->m.ScriptFile.c_str(), Cmd.Name().c_str(), Params[0].c_str()) );
				}
				else
				{
	...
	//************************** SETSTATUS ***************************
		...
		if( Params.size() >= 2 )
		{
			...
			int Flags = 0;
			if( Flag.contains("swimming") ) SetBits( Flags, PLAYER_MOVE_SWIMMING );
			if( Flag.contains("sitting") ) SetBits( Flags, PLAYER_MOVE_SITTING ); //JAN2010_09 - attempting to allow inv while sitting again

			if( Flags )
			{
				if( Action == "add" ) SetBits( m_StatusFlags, Flags );
				else ClearBits( m_StatusFlags, Flags );
			}
		}
		else ERROR_MISSING_PARMS;
	}
	...
	//************************** SETIDLEANIM **************************
		...
		if( Params.size() >= 1 )
		{
			if( Params[0] == "none" ) m_IdleAnim = "";
			else m_IdleAnim = Params[0];
		}
		else ERROR_MISSING_PARMS;
	}
	//************************** SETMOVEANIM **************************
	...
	//***************************** PLAYANIM **************************
		...
		if( Params.size() >= 1 )
		{
			...
			if( !stricmp(pszAnimType,"move") ) AnimType = MONSTER_ANIM_WALK;
			else if( !stricmp(pszAnimType,"once") ) AnimType = MONSTER_ANIM_ONCE;
			else if( !stricmp(pszAnimType,"hold") ) AnimType = MONSTER_ANIM_HOLD;
			else if( !stricmp(pszAnimType,"critical") ) 
			{
				AnimType = MONSTER_ANIM_ONCE;
				Priority = true;
			}
			else if( !stricmp(pszAnimType,"break") ) BreakAnimation( MONSTER_ANIM_BREAK, "", NULL );
	...
	else if( Cmd.Name() == "setanimtorso" || Cmd.Name() == "setanimlegs" ) { // Player only
		if( Params.size() >= 1 )
		{
			if( IsPlayer() )
		...
		if( Params.size() >= 1 )
		{
			...
			else if( Params.size() >= 2 )
			{
				...
				else
				{
					...
					if( pEntity )
					{
						bool fFlee = false; fMove = true;
						if( Params.size() >= 3 && Params[2].contains("flee") ) fFlee = true;
						...
						if( !fFlee )	//Set dest to target
						{
							...
							else NewDest.Origin = pEntity->EyePosition();
...
bool CMSMonster::GetScriptVar( msstring &ParserName, msstringlist &Params, CScript *BaseScript, msstring &Return )
{
		...
		if( ParserName == "$cansee" && Params.size() >= 1 )
		{
			...
			if( pSpecificEnt ) SightedList.add( pSpecificEnt );
			 ...
			 for (int i = 0; i < SightedList.size(); i++) 
			{
				...
				*/
				...
				if( pSighted->pev->deadflag != DEAD_NO ) continue;
...
msstring_ref CMSMonster::GetProp( CBaseEntity *pTarget, msstring &FullParams, msstringlist &Params )
{
	...
	if( pMonster && Prop == "walkspeed" ) RETURN_FLOAT( pMonster->WalkSpeed( true ) )
	else if( pMonster && Prop == "runspeed" ) RETURN_FLOAT( pMonster->RunSpeed( true ) )
	else if( pMonster && Prop == "movespeed" ) RETURN_FLOAT( pMonster->m_flGroundSpeed ) //Thothie MAR2008 - attempting to fix freeze_solid issues
	else if( pMonster && Prop == "framerate" ) RETURN_FLOAT( pMonster->m_Framerate ) //Thothie MAR2008 - attempting to fix freeze_solid issues
