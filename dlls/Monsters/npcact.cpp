...
void NPCScript :: Act( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	//Print("->IsAlive()2");
	if( !pMonster->IsAlive() ) return;
	//Don't stop an attacking monster or
	//a monster already in a script
	//Print("->MONSTERSTATE_SCRIPT");
    if( pMonster->m_MonsterState == MONSTERSTATE_SCRIPT ) return;
	//Print("->m_fStopAI");
	if( !m_fStopAI && pMonster->m_hEnemy != NULL ) return;
...
void NPCScript :: MoveThink( )
{
	...
	if( !pMonster->HasConditions( MONSTER_HASMOVEDEST ) )
	{
		...
		switch( m_iType ) {
		case SCRIPT_MOVE_PLAYANIM: PlayAnim( ); break;
		case SCRIPT_MOVE_RUNEVENT: RunScriptEvent( ); break;
...
void NPCScript :: FireTarget( )
{
	...
	if( pMonster )
	{
		...
		if( pMonster->m_hEnemy == this ) pMonster->m_hEnemy = NULL;
		m_NPC = NULL;
	}
	...
	if( FireEvent ) FireTargets( STRING(FireEvent), pMonster, this, USE_TOGGLE, 0 );

	if( m_fShouldExpire ) DelayedRemove( );
}
