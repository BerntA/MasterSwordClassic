...
void CBaseEntity :: SUB_Remove( void )
{
	...
	//Master Sword - Deallocate mod memory the entity used
...
void CBaseDelay :: KeyValue( KeyValueData *pkvd )
{
	...
	{
		CScriptedEnt::KeyValue( pkvd );	//Master Sword - now scripted
	}
}
...
void FireTargets( const char *targetName, CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( strlen(targetName) < 1 ) return; //OCT2007a - attempting to strip null map events
...
void CBaseDelay :: SUB_UseTargets( CBaseEntity *pActivator, USE_TYPE useType, float value )
{
	...
	//
	//Thothie OCT2007a - FStringNull(pev->target) does not seem to be working
	...
	if (m_flDelay != 0)
	{
		...
		pTemp->pev->enemy = edict(); //Used like owner, but owner is taken
...
CBaseEntity *CBaseDelay::GetSelf( )
{
	if( pev->enemy )
		return CBaseEntity::Instance( pev->enemy );
	return this;
}
