...
CSoundEnt *pSoundEnt = &CSoundEnt::m_gSoundEnt;
CSoundEnt CSoundEnt::m_gSoundEnt;
...
void CSoundEnt :: InsertSound( CBaseEntity *pSrcEntity, msstring_ref Type, const Vector &vecOrigin, int iVolume, float flDuration, float flDangerRadius )
{
	...
	pSoundEnt->m_SoundPool[ idx ].m_SrcEntity = pSrcEntity;
	pSoundEnt->m_SoundPool[ idx ].m_Type = Type;
	pSoundEnt->m_SoundPool[ idx ].m_DangerRadius = flDangerRadius;	
}
//------------

int CSoundEnt :: InsertSound ( int iType, const Vector &vecOrigin, int iVolume, float flDuration )
{
	int	iThisSound;
