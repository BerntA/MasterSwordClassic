...
int UTIL_EntitiesInBox( CBaseEntity **pList, int listMax, const Vector &mins, const Vector &maxs, int flagMask, int startIndex )
{
	...
	if ( !pEdict || !pEdict->pvPrivateData ) //MAR2008b - attempt to fix the revenge of the chatbug
	{
		...
		//Death to chatbug, etc!
		//MiB Feb2008 - Recursively call until you find a proper entity
		return UTIL_EntitiesInBox( pList , listMax, mins , maxs , flagMask, startIndex + 1 );
		//return count;
	}
...
int UTIL_MonstersInSphere( CBaseEntity **pList, int listMax, const Vector &center, float radius, int startIndex )
{
	...
	if ( !pEdict || !pEdict->pvPrivateData ) //MAR2008b - zapping neo-chatbug
	{
		//Death to chatbug, etc!
		//MiB Feb2008 - Recursively call until you find a proper entity
		return UTIL_MonstersInSphere( pList , listMax, center , radius, startIndex + 1 );
		//return count;
	}
...
void UTIL_DoTokenScriptEvent( const char *tokenevents, CBaseEntity *pTarget )
{
	...
	if ( m_scriptevent.starts_with("gm_") )
	{
		...
		Params.clearitems( );
		if ( pTarget ) Params.add( EntToString(pTarget) );
		...
		if ( Tokens.size() > 1 )
		{
			...
			{
				if ( i > 0 ) Params.add( Tokens[i].c_str() );
			}
		}
	...
	else
	{
		...
		if ( pTarget ) pScripted=pTarget->GetScripted();
		if ( pScripted )
		{
			//msstringlist Parameters; //send eventparams (to do)
			//Thothie JAN2011_25 allow adding of params to trigger-script events
			static msstringlist Tokens;
			Tokens.clearitems();
			TokenizeString( m_scriptevent.c_str(), Tokens );
			static msstringlist Params;
			...
			pScripted->CallScriptEvent( Tokens[0].c_str(),&Params);
		}
	}
}
//[end] NOV2014_13 Thothie - centralizing trigger scriptevent and req functions

//DEC2014_21 Thothie - Centralizing music/time conversion
float UTIL_StringToSecs( const char *timein )
{
	float out_secs;
	msstring mstimein = timein;

	if ( mstimein.contains(":") )
	{
		msstring SongMinutes = mstimein.thru_char(":");
		msstring SongSeconds = mstimein.substr( mstimein.findchar(":") + 1 );
		out_secs = (atof(SongMinutes) * 60.f) + atof(SongSeconds);
		//Print("DEBUG: UTIL_StringToSecs in: %s = [%s mins %s secs] = %f\n",mstimein.c_str(),SongMinutes.c_str(),SongSeconds.c_str(),out_secs);
		//logfile << UTIL_VarArgs("DEBUG: UTIL_StringToSecs in: %s = [%s mins %s secs] = %f\n",mstimein.c_str(),SongMinutes.c_str(),SongSeconds.c_str(),out_secs);
	}
	else
	{
		out_secs = atof(timein) * 60.f;
	}
...
//NOV2014_16 Thothie - simplfying code side array management (create/edit)
...
int UTIL_NumPlayers()
{
	...
	{
		int icvar_getfakeplayers = atoi( EngineFunc::CVAR_GetString("ms_fake_players") );
		if ( icvar_getfakeplayers > 0 )  total_valid_players = icvar_getfakeplayers;
	}
	//shouldn't need an invalidated flag here, shouldn't be possible to come up with a false zero
...
int UTIL_NumActivePlayers()
{
	...
	if ( total_valid_players <= 0 && flagged_invalid ) total_valid_players = 1; //in case we flagged all the players invalid, dun wanna screw with mob spawns/chests
	else if ( total_valid_players < 0  ) total_valid_players = 1; //redundancy, not sure how this wouldhappen
	return total_valid_players;
}
...
float UTIL_TotalHP()
{
	...
	for( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( pPlayer )
		{
			...
			{
				++total_valid_players;
				total_hp=total_hp+pPlayer->MaxHP();
			}
		}
	}
	if ( atoi(EngineFunc::CVAR_GetString("ms_central_enabled")) == 0 )
	...
	if ( total_hp <= 0 && flagged_invalid ) total_hp = 1; //in case we flagged all the players invalid, dun wanna screw with mob spawns/chests
	else if ( total_valid_players < 0  ) total_hp = 1; //redundancy, not sure how this wouldhappen
	return total_hp;
}
...
float UTIL_AvgHP()
{
	...
	if ( out <= 0 && flagged_invalid ) out = 1; //in case we flagged all the players invalid, dun wanna screw with mob spawns/chests
	else if ( out < 0  ) out = 1; //redundancy, not sure how this wouldhappen
	return out;
}
...
void UTIL_ShowMessage( const char *pString, CBaseEntity *pEntity )
{
	...
	//UTIL_SayText( pString, pEntity ); //msg bug? - Thothie
