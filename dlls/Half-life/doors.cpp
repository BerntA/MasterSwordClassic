...
void CBaseDoor::Blocked( CBaseEntity *pOther )
{
	...
	//Thothie AUG2011_29
	//doors that are "unblockable" do damage, but do not stop
	bool door_unblockable = false;
	if ( FBitSet ( pev->spawnflags, SF_DOOR_UNBLOCKABLE ) ) door_unblockable = true;

	// if a door has a negative wait, it would never come back if blocked,
	// so let it just squash the object to death real fast

	//AUG2013_21 Tim Smith - fixes some sound bugs with doors
	if ( m_flWait >= 0 )
	{
		if ( !FBitSet( pev->spawnflags, SF_DOOR_SILENT ) )
			STOP_SOUND(ENT(pev), CHAN_STATIC, (char*)STRING(pev->noiseMoving) );
	...
	if (m_flWait >= 0 && !door_unblockable )
	{
	...
	if ( !FStringNull ( pev->targetname ) && !door_unblockable )
	{
		for (;;)
		{
