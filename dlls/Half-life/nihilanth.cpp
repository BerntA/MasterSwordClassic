...
void CNihilanth ::Spawn(void)
{
	...
	if (m_szRechargerTarget[0] == '\0')
		 strncpy(m_szRechargerTarget,  "n_recharger", sizeof(m_szRechargerTarget) );
	if (m_szDrawUse[0] == '\0')
		 strncpy(m_szDrawUse,  "n_draw", sizeof(m_szDrawUse) );
	if (m_szTeleportUse[0] == '\0')
		 strncpy(m_szTeleportUse,  "n_leaving", sizeof(m_szTeleportUse) );
	if (m_szTeleportTouch[0] == '\0')
		 strncpy(m_szTeleportTouch,  "n_teleport", sizeof(m_szTeleportTouch) );
	if (m_szDeadUse[0] == '\0')
		 strncpy(m_szDeadUse,  "n_dead", sizeof(m_szDeadUse) );
	if (m_szDeadTouch[0] == '\0')
		 strncpy(m_szDeadTouch,  "n_ending", sizeof(m_szDeadTouch) );
...
void CNihilanth ::NextActivity()
{
	...
	if ((pev->health < gSkillData.nihilanthHealth / 2 || m_iActiveSpheres < N_SPHERES / 2) && m_hRecharger == NULL && m_iLevel <= 9)
	{
		...
		float flDist = 8192;
		 ...
		 _snprintf(szName, sizeof(szName),  "%s%d",  m_szRechargerTarget,  m_iLevel );
		...
		if (flDist < 128.0)
		{
			...
			if (iseq != pev->sequence)
			{
				char szText[64];
				 ...
				 _snprintf(szText, sizeof(szText),  "%s%d",  m_szDrawUse,  m_iLevel );
			...
			else
			{
				...
				else
				{
					char szText[64];
					 ...
					 _snprintf(szText, sizeof(szText),  "%s%d",  m_szTeleportTouch,  m_iTeleport );
					CBaseEntity *pTouch = UTIL_FindEntityByTargetname(NULL, szText);

					 _snprintf(szText, sizeof(szText),  "%s%d",  m_szTeleportUse,  m_iTeleport );
