...
int DispatchSpawn( edict_t *pent )
{
	...
	if( pEntity )
	{
		...
		//Set_DispatchSpawnDbg( "Call Spawn" );
		pEntity->Spawn();
		//return;
...
void DispatchTouch( edict_t *pentTouched, edict_t *pentOther )
{
	...
	catch( ... ) { MSErrorConsoleText( "DispatchTouch", DispatchTouchPrg ); }
}
...
void DispatchThink( edict_t *pent )
{
	...
	if( pEntity )
	{
		if( FBitSet( pent->v.playerclass, ENT_EFFECT_FOLLOW_ROTATE ) )
		{
			CBaseEntity *pOwner = pEntity->m_AttachToEnt.Entity();
			if( pOwner )
			{
				pEntity->pev->origin = pOwner->pev->origin;
				if( FBitSet( pent->v.playerclass, ENT_EFFECT_FOLLOW_ALIGN_BOTTOM ) )
					pEntity->pev->origin.z += pOwner->pev->mins.z;
			}
			else
				pEntity->m_AttachToEnt = NULL;
		}

		//Set_DispatchThinkDbg( "Check Dormant" );
