...
// THOTHIE: Just the ambient_generic entity, I left the rest alone
...
LINK_ENTITY_TO_CLASS( ambient_generic, CAmbientGeneric ); //Thothie - attempting to build new amb generic ms MSMapEnts - so this needs to be altered
...
{
	//Thothie attempting to fix Ambient_Generic ==================== BEGIN MAY2007a

	if ( !m_fActive )
	{
		//m_fActive = true;
		//ALERT( at_console,"Activated" );
		//this is kinda working
		//EMIT_SOUND_DYN(edict(), 2, STRING(pev->message), 0.9, ATTN_NORM, 0, 100); //Thothie - force play
		float inten_vol = ( pev->health ) * 0.1;

		UTIL_EmitAmbientSound(ENT(pCaller->pev), pev->origin, STRING(pev->message), inten_vol, m_flAttenuation, 0, 100);
	}
	else if ( m_fActive )
	{
		//never gets here
	...
	//Thothie attempting to fix Ambient_Generic ==================== END
...
{
	//if ( pitch != 100 ) Print("DEBUG: EMIT_SOUND_DYN pitch %f %i\n",pitch,(int)pitch);
...
// hit the world, try to play sound based on texture material type
	...
	case CHAR_TEX_VENT:	fvol = 0.5; fvolbar = 0.3;
		...
		cnt = 2;
		break;
	case CHAR_TEX_SNOW:	fvol = 0.9; fvolbar = 0.6; //MAR2008a Thothie
