...
extern int			UTIL_MonstersInSphere( CBaseEntity **pList, int listMax, const Vector &center, float radius, int startIndex = 1 );
extern int			UTIL_EntitiesInBox( CBaseEntity **pList, int listMax, const Vector &mins, const Vector &maxs, int flagMask, int startIndex = 1 );
...
extern void			UTIL_DoTokenScriptEvent( const char *tokenevents, CBaseEntity *pTarget ); //NOV2014_13 Thothie - centralizing trigger scriptevent
...
//DEC2014_04 Thothie - changed pitch from int to float (think fubar), also note, seems you can only change the pitch of unsigned waves. (Possibly also requires 8-bits.)

void EMIT_SOUND_DYN(edict_t *entity, int channel, const char *sample, float volume, float attenuation,
						   int flags, float pitch);


inline void EMIT_SOUND(edict_t *entity, int channel, const char *sample, float volume, float attenuation)
{
	//Thothie AUG2007 - getting desperate to figure what's causing the cave bat death = crash
...
//DEC2012_11 - Thothie - Allow pitch adjustment
inline void EMIT_SOUND2(edict_t *entity, int channel, const char *sample, float volume, float attenuation, float pitch)
{
	if ( volume > 1 ) volume = 1;
	if ( volume < 0 ) volume = 0;
	//[/Thothie]
