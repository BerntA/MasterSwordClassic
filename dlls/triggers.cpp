...
public:
	...
	msstring m_scriptevent; //DEC2012_20 Thothie - Adding Trigger scriptevent to trigger_relay
	string_t ms_master; //FEB2015_12 Thothie - Adding master to trigger_relay

private:
	USE_TYPE	triggerType;
	float m_flRandomChance; //Master Sword
};
LINK_ENTITY_TO_CLASS( trigger_relay, CTriggerRelay );
LINK_ENTITY_TO_CLASS( mstrig_relay, CTriggerRelay );
...
void CTriggerRelay::KeyValue( KeyValueData *pkvd )
{
	...
	{
		m_flRandomChance = atof( pkvd->szValue );
	}
	else
		CBaseDelay::KeyValue( pkvd );
}
...
void CTriggerRelay::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if ( ms_master )
	{
		...
		if (!UTIL_IsMasterTriggered(ms_master,pActivator))
		{
	...
	{
		//Thothie DEC2012_20 - Copypasta from triggers so ent has trigger based properties
		if ( m_scriptevent.len() ) UTIL_DoTokenScriptEvent(m_scriptevent.c_str(),pActivator); //NOV2014_13 Thothie - Moved to utils.cpp
...
LINK_ENTITY_TO_CLASS( mstrig_multi, CMultiManager );
...
void CMultiManager :: ManagerThink ( void )
{
	...
	while ( m_index < m_cTargets && m_flTargetDelay[ m_index ] <= time )
	{
		...
		if( fFireRandom )
		{
			//Only fire the one, randomly-chosen target
			m_index = m_cTargets;
			break;
		}
		m_index++;
...
CMultiManager *CMultiManager::Clone( void )
{
	...
	pMulti->fFireRandom = fFireRandom;
...
public:
	...
	msstring m_scriptevent; //FEB2010_23 Thothie - Trigger scriptevent

	msstring m_reqhp; //AUG2011 Thothie - Trigger Reqhp
	msstring m_reqavghp; //AUG2011 Thothie - Trigger Reqhp
	msstring m_players; //AUG2011 Thothie - Trigger Reqplayers
	msstring m_else_target; //AUG2011 Thothie - Fire this target if above tests fail
...
{
	//Thothie FEB2010_23 - call scriptevent on target
	if  (FStrEq(pkvd->szKeyName, "scriptevent"))
	{
		m_scriptevent = pkvd->szValue;
		pkvd->fHandled = TRUE;
	...
	//- this can be used on any trigger from here, but should only apply to trigger_multiple
	//0=none, 1=scriptevents, 2=scriptevents&triggers
	...
	//- apply scripts to all in bounds, in case you unlock a large trigger_once with a script_event
	//0=normal 1=run events on all in box bounds
	if  (FStrEq(pkvd->szKeyName, "eventallinbounds"))
	{
...
void CBaseTrigger :: HurtTouch ( CBaseEntity *pOther )
{
	...
	//DEC2012_12 Thothie - stop fragging pets!
	if ( pev->dmg > 50 )
	{
		IScripted *pScripted = pOther->GetScripted();
		if ( pScripted )
		{
			if ( atoi(pScripted->GetFirstScriptVar("IS_COMPANION")) > 0 )
			{
				static msstringlist Params;
				Params.clearitems( );
				Params.add( "trigger_hurt" );
				pScripted->CallScriptEvent("companion_unsummon",&Params);
				return;
			}
		}
	}
	...
	//Thothie FEB2015_04 - implement scriptevent feature
	if ( m_scriptevent.len() > 0 && ( pOther->IsMSMonster() || pOther->IsPlayer() ) )
	{
		UTIL_DoTokenScriptEvent(m_scriptevent.c_str(),pOther);
	}

	static msstringlist Params;
	Params.clearitems( );
	Params.add( EntToString(pOther) );
 	CallScriptEvent( "game_dodamage", &Params );
...
//	else
		//{
	//DEC2012 - Thothie - Trying to figure a way to get trigger_multi to affect multiple targets
	//- the comments here (made my Dogg, I guess) just wrecked the brackets... Cleaning it didn't fix (or break) anything though
...
//moved to UTIL_TotalHP
/*
float thoth_TotalHP( void )
{
	float num = 0;
	...
	for( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( pPlayer ) num = num + pPlayer->MaxHP();
	}
	if ( atoi(EngineFunc::CVAR_GetString("ms_central_enabled")) == 0 )
	{
		float fakehp = atof(EngineFunc::CVAR_GetString("ms_fake_hp"));
		if ( fakehp > 0 ) num = fakehp;
	}
	return num ;
}
*/
...
float thoth_AvgHP( void )
{
	...
	if ( numplayers > 0 ) num = num / numplayers;
...
void CBaseTrigger :: ActivateMultiTrigger( CBaseEntity *pActivator )
{
	...
	if ( trig_individualized ) bNdvCdwn = true;
	...
	if ( bNdvCdwn )
	{
		if ( gpGlobals->time < trig_actcooldown )
		{
			//note that it's his next cooldown, but not mine
	...
	// Original code, used if bNdvCdwn is false
	...
	//original, pre-individualized trigger code:
	...
	if (!FStringNull(pev->noise))
		EMIT_SOUND(edict(), CHAN_VOICE, (char*)STRING(pev->noise), 1, ATTN_NORM); //Thothie MAY2007 - changed  edict() from (ENT(pev) - teleporter sounds never play
...
// pev->takedamage = DAMAGE_NO;
	...
	if ( m_reqhp.len() > 0 )
	{
		...
		if ( reqhp_stringlist.size() > 0 ) thoth_min_hp = atof(reqhp_stringlist[0].c_str());
		if ( reqhp_stringlist.size() > 1 )
		{
			thoth_max_hp = atof(reqhp_stringlist[1].c_str());
			if ( thoth_max_hp < thoth_min_hp ) thoth_max_hp = 0;
			if ( thoth_min_hp == 0 ) thoth_min_hp = 1;
		}

		float thoth_hp = UTIL_TotalHP();
		if ( thoth_hp < thoth_min_hp ) use_else_target = true;
		if ( thoth_hp > thoth_max_hp && thoth_max_hp > 0 ) use_else_target = true;
	}

	//AUG2011_17 Thothie - Filter by avg HP
	if ( m_reqavghp.len() > 0 )
	{
		msstringlist reqavghp_stringlist;
		TokenizeString(m_reqavghp,reqavghp_stringlist);

		if ( reqavghp_stringlist.size() > 0 ) thoth_min_hp = atof(reqavghp_stringlist[0].c_str());
		...
		if ( thoth_tavghp < thoth_min_hp ) use_else_target = true;
		if ( thoth_tavghp > thoth_max_hp && thoth_max_hp > 0 ) use_else_target = true;
	}

	//AUG2011_17 Thothie - Filter by #players
	if ( m_players.len() > 0 )
	{
		...
		if ( reqplayers_strlist.size() > 0 ) min_players = atoi(reqplayers_strlist[0].c_str());
		if ( reqplayers_strlist.size() > 1 )
		{
			max_players = atoi(reqplayers_strlist[1].c_str());
		...
		for( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			CBaseEntity *pEntity = UTIL_PlayerByIndex( i );
			CBasePlayer *pPlayer = (CBasePlayer *)pEntity;
			if ( pPlayer ) thoth_total_players = thoth_total_players + 1;
		}
#if !TURN_OFF_ALERT
		if ( atoi(EngineFunc::CVAR_GetString("ms_central_enabled")) == 0 )
		{
			int fakeplayers = atoi(EngineFunc::CVAR_GetString("ms_fake_players"));
			if ( fakeplayers > 0 ) thoth_total_players = fakeplayers;
		}
#endif
		*/
		if ( thoth_total_players < min_players ) use_else_target = true;
		if ( thoth_total_players > max_players && max_players > 0 ) use_else_target = true;
	}

	//Thothie NOV2014_19 individualized trigger application:
	//skip firing triggers if my actual cooldown isn't achieved
	//but still apply script events to new toucher, if trig_individualized = 1
	bool skip_triggers = false;
	if ( bNdvCdwn && trig_secondary_activation && trig_individualized == 1 ) skip_triggers = true;
	...
	if ( !trig_secondary_activation ) trig_actcooldown = (gpGlobals->time + m_flWait) - 0.1;
	...
	if ( !skip_triggers )
	{
		...
		if ( use_else_target )
		{
			...
			{
				m_hActivator = pActivator;
				FireTargets( m_else_target.c_str(), pActivator, this, USE_TOGGLE, 0 );
			}
		}
		else
		{
	...
	//Thothie FEB2010_23 - trigger scriptevent
	if ( m_scriptevent.len() > 0 && ( pActivator->IsMSMonster() || pActivator->IsPlayer() ) )
	{
		UTIL_DoTokenScriptEvent(m_scriptevent.c_str(),pActivator); //NOV2014_13 Thothie - Moved to utils.cpp	}

		//Thothie NOV2014_19 - apply scriptevent to all in bounds, option
		msstring myclass = STRING(pev->classname); //gotta be a better way to convert this shit
		//Print("DEBUG: classname %s allinb %s\n",myclass.c_str(),trig_eventallinbounds ? "1" : "0");
		if ( myclass.starts_with("trigger_once") && trig_eventallinbounds )
		{
			...
			if ( !(pev->spawnflags & SF_TRIGGER_NOCLIENTS) && !(pev->spawnflags & SF_TRIGGER_ALLOWMONSTERS) ) tcount = UTIL_EntitiesInBox( pList, 255, pev->absmin, pev->absmax, FL_CLIENT );
			else if ( !(pev->spawnflags & SF_TRIGGER_NOCLIENTS) && (pev->spawnflags & SF_TRIGGER_ALLOWMONSTERS) ) tcount = UTIL_EntitiesInBox( pList, 255, pev->absmin, pev->absmax, FL_CLIENT|FL_MONSTER );
			else if ( (pev->spawnflags & SF_TRIGGER_NOCLIENTS) && (pev->spawnflags & SF_TRIGGER_ALLOWMONSTERS) ) tcount = UTIL_EntitiesInBox( pList, 255, pev->absmin, pev->absmax, FL_MONSTER );
			 for (int i = 0; i < tcount; i++) 
			{
				//Print("DEBUG: trigger_once running %s on %s\n", m_scriptevent.c_str(), pList[i]->m_DisplayName.c_str());
				if ( pList[i] != pActivator ) UTIL_DoTokenScriptEvent(m_scriptevent.c_str(),pList[i]);
			}
		}
	}
	...
	if (m_flWait > 0)
	{
		...
		//original pre invidiualized trigger code:
...
LINK_ENTITY_TO_CLASS( ms_counter, CTriggerCounter );
...
LINK_ENTITY_TO_CLASS( mstrig_push, CTriggerPush ); //Thothie AUG2007 - to get around FGD conflicts
...
void CTriggerPush :: Touch( CBaseEntity *pOther )
{
	...
	//Thothie AUG2007 - Trigger push - players only
	if ( (pev->spawnflags & SF_TRIGGER_HURT_CLIENTONLYTOUCH) && !pOther->IsPlayer() )
	{
		// this trigger is only allowed to touch clients, and this ain't a client.
		return;
	...
	//Thothie AUG2007 - Trigger push - monsters only
	if ( (pev->spawnflags & SF_TRIGGER_HURT_NO_CLIENTS) && pOther->IsPlayer() )
	{
		// this trigger is only allowed to touch monsters, and this ain't a monster.
		return;
	}
	...
	if ( pevToucher->solid != SOLID_NOT && pevToucher->solid != SOLID_BSP )
	{
		...
		if ( pOther->IsPlayer() )
		{
			...
			if ( pPlayer ) pPlayer->SetScriptVar("PLR_LAST_PUSH",FloatToString(gpGlobals->time));
		}
...
void CBaseTrigger :: TeleportTouch( CBaseEntity *pOther )
{
	...
	if ( m_flDelay > 0 )
	{
		if ( thoth_last_triggered > 0 && ( (thoth_last_triggered + m_flDelay) > gpGlobals->time ) ) return;
	}
...
*/
	...
	if ( use_else_target )
	{
		...
		{
			m_hActivator = pOther;
			FireTargets( m_else_target.c_str(), pOther, this, USE_TOGGLE, 0 );
		}
	}
	else
	{
	...
	//Thothie DEC2012_15 - Copy-pasta scriptevent function here (standard trigger functions do not work)
	if ( m_scriptevent.len() > 0 && ( pOther->IsMSMonster() || pOther->IsPlayer() ) )
	{
		UTIL_DoTokenScriptEvent(m_scriptevent.c_str(),pOther); //NOV2014_13 Thothie - Moved to utils.cpp	}
	}
	
	//MiB JAN2010_21 - Removing this to make teleporting go to a random tele destination
	//					of the right target.
	/*pentTarget = FIND_ENTITY_BY_TARGETNAME( pentTarget, STRING(pev->target) );
	if (FNullEnt(pentTarget))
	   return;*/
	...
	while ( pLoc = UTIL_FindEntityByClassname( pLoc, "info_teleport_destination" ) )
	{
		...
		if ( msstring( STRING(pev->target) ) == msstring( STRING(pLoc->pev->targetname) ) )
			TeleLocs.add( pLoc );
	}
	...
	//pentTarget = TeleLocs[ RANDOM_LONG( 0 , TeleLocs.size()-1 ) ]->edict();
	//Thothie OCT2011_28 - cycle teleport destinations, instead of random
