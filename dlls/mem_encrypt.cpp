...
void safemem_varbase::Alloc( )
{
	...
	if( m_DataBuf ) delete m_DataBuf;				//Delete old buffer
...
void safemem_varbase::Encrypt( )
{
	...
	if( !Size ) return;								//Size is zero - should never happen
...
void safemem_varbase::UnRegister( ) 
{
	...
	#ifdef VALVE_DLL
		if( pData ) delete pData;						//Delete the mem
		if( m_DataBuf ) delete m_DataBuf;				//Delete the buffer
	#endif
