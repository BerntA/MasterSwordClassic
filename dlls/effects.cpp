...
void CBeam::BeamDamage(TraceResult *ptr)
{
	...
	if (ptr->flFraction != 1.0 && ptr->pHit != NULL)
	{
		...
		{
			ClearMultiDamage();
			pHit->TraceAttack(NULL, pev, pev->dmg * (gpGlobals->time - pev->dmgtime), (ptr->vecEndPos - pev->origin).Normalize(), ptr, DMG_ENERGYBEAM);
...
void CGibShooter ::Precache(void)
{
	if (g_Language == LANGUAGE_GERMAN)
	{
		...
		//thothie - wtf are Germany gibs? - it's not in the preache so removing
	}
	else
	{
		m_iGibModelIndex = PRECACHE_MODEL("models/hgibs.mdl");
...
{
	PRECACHE_MODEL("models/rockgibs.mdl"); //thothie substituting cans for rock gibs to cut down on item limit
...
void CItemSoda::Spawn(void)
{
	...
	SET_MODEL(ENT(pev), "models/rockgibs.mdl"); //thothie cans to rock gibs, item limit
