...
void CGrenade::Explode(TraceResult *pTrace, int bitsDamageType)
{
	...
	{
		WRITE_SHORT(g_sModelIndexExplosion);
	}
	else
	{
...
void CGrenade::BounceTouch(CBaseEntity *pOther)
{
	...
	if (m_flNextAttack < gpGlobals->time && pev->velocity.Length() > 100)
	{
		...
		if (pevOwner)
		{
			...
			ClearMultiDamage();
			pOther->TraceAttack(pev, pevOwner, 1, gpGlobals->v_forward, &tr, DMG_CLUB);
...
CGrenade *CGrenade::ShootTimed(entvars_t *pevOwner, Vector vecStart, Vector vecVelocity, float time)
{
	...
	SET_MODEL(ENT(pGrenade->pev), "models/rockgibs.mdl"); //thothie - item limit - don't need w_grenade.mdl - substituting
...
CGrenade *CGrenade ::ShootSatchelCharge(entvars_t *pevOwner, Vector vecStart, Vector vecVelocity)
{
	...
	SET_MODEL(ENT(pGrenade->pev), "models/rockgibs.mdl"); // Change this to satchel charge model - thothie, don't need grenade.mdl substituting, item limit
