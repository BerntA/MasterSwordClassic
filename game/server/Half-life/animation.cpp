...
/*char LookupSequenceName( void *pmodel, const int *inseqindex )
{
	...
	return STRING(pseqdesc[inseqindex].label);
...
{
	//Thothie APR2010_19 - MSC doesn't use sound anim events, so don't waste time tripping the system for them
	/*
...
void SequencePrecache(void *pmodel, const char *pSequenceName)
{
	...
	if (index >= 0)
	{
		...
		for (int i = 0; i < pseqdesc->numevents; i++)
		{
			...
			// Thothie APR2010_19 - don't bother with this if we ain't gonna use it
			/*
