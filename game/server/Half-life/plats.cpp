...
void CFuncPlat ::HitTop(void)
{
	...
	//MAR2010_10 - Thothie - adjust down 16 units if flagged for stuck bug
	if (IsStuckAdjust())
	{
		Vector vecDest = pev->origin;
		vecDest += Vector(0, 0, -16);
		pev->origin = vecDest;
	}
...
void CFuncTrackTrain ::Next(void)
{
	...
	//APR2008a Thothie - No Yaw option for Tracktrain
	//- problem, this is affecting all trains, not just those with the flag
	if (!(pev->spawnflags & SF_TRACKTRAIN_NOYAW))
	{
