...
#include "../client/cl_util.h"

#include <assert.h>
#include <string.h>
#include <stdio.h>

#include "../client/parsemsg.h"
#include "../client/hud_servers.h"
...
{
	char str[256];
	_snprintf(str, sizeof(str), "Ban %d: %2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x\n", g_BannedPlayerPrintCount++,
		id[0], id[1], id[2], id[3],
		id[4], id[5], id[6], id[7],
	...
	Panel **pParentPanel)
{
	...
	size_t strLength = (strlen(pchGameDirT) + 1);
	m_pchGameDir = (char *)malloc(strLength);
	strncpy(m_pchGameDir, pchGameDirT, strLength);
...
void CVoiceStatus::CreateEntities()
{
	...
	for (int i = 0; i < VOICE_MAX_PLAYERS; i++)
	{
		...
		if (pClient == localPlayer && !CL_IsThirdPerson())
			continue;
...
void CVoiceStatus::UpdateServerState(bool bForce)
{
	...
	if (bForce || m_bServerModEnable != bCVarModEnable)
	{
		...
		{
			char msg[256];
			_snprintf(msg, sizeof(msg), "CVoiceStatus::UpdateServerState: Sending '%s'\n", str);
			gEngfuncs.pfnConsolePrint(msg);
		}
	}

	char str[2048];
	strncpy(str, "vban", sizeof(str));
	bool bChange = false;
	...
	for (unsigned long dw = 0; dw < VOICE_MAX_PLAYERS_DW; dw++)
	{
		...
		char numStr[512];
		_snprintf(numStr, sizeof(numStr), " %x", banMask);
		strncat(str, numStr, strlen(numStr));
	}
...
void CVoiceStatus::HandleVoiceMaskMsg(int iSize, void *pbuf)
{
	...
	for (dw = 0; dw < VOICE_MAX_PLAYERS_DW; dw++)
	{
		...
		if (gEngfuncs.pfnGetCvarFloat("voice_clientdebug"))
		{
			...
			_snprintf(str, sizeof(str), "    - m_AudiblePlayers[%d] = %lu\n", dw, m_AudiblePlayers.GetDWord(dw));
			gEngfuncs.pfnConsolePrint(str);

			_snprintf(str, sizeof(str), "    - m_ServerBannedPlayers[%d] = %lu\n", dw, m_ServerBannedPlayers.GetDWord(dw));
			gEngfuncs.pfnConsolePrint(str);
		}
	}
...
void CVoiceStatus::FreeBitmaps()
{
	...
	if (m_pLocalBitmap) delete m_pLocalBitmap;
	m_pLocalBitmap = NULL;

	if (m_pAckBitmap) delete m_pAckBitmap;
	m_pAckBitmap = NULL;

	if (m_pSpeakerLabelIcon) delete m_pSpeakerLabelIcon;
	m_pSpeakerLabelIcon = NULL;

	if (m_pScoreboardNeverSpoken) delete m_pScoreboardNeverSpoken;
	m_pScoreboardNeverSpoken = NULL;

	if (m_pScoreboardNotSpeaking) delete m_pScoreboardNotSpeaking;
	m_pScoreboardNotSpeaking = NULL;

	if (m_pScoreboardSpeaking) delete m_pScoreboardSpeaking;
	m_pScoreboardSpeaking = NULL;

	if (m_pScoreboardSpeaking2) delete m_pScoreboardSpeaking2;
	m_pScoreboardSpeaking2 = NULL;

	if (m_pScoreboardSquelch) delete m_pScoreboardSquelch;
	m_pScoreboardSquelch = NULL;

	if (m_pScoreboardBanned) delete m_pScoreboardBanned;
	m_pScoreboardBanned = NULL;
...
void CVoiceStatus::SetPlayerBlockedState(int iPlayer, bool blocked)
{
	...
	{
		char str[256];
		_snprintf(str, sizeof(str), "CVoiceStatus::SetPlayerBlockedState: setting player %d ban to %d\n", iPlayer, !m_BanMgr.GetPlayerBan(playerID));
		gEngfuncs.pfnConsolePrint(str);
	}
...
//Master Sword
#include "../client/vgui_TeamFortressViewport.h"
#include "../client/vgui_ScorePanel.h"
...
void CHLVoiceStatusHelper::UpdateCursorState()
{
