...
{
	lastMoveRightUp = gpGlobals->time;
	KeyUp(&in_moveright);
...
void DLLEXPORT CL_CreateMove(float frametime, struct usercmd_s *cmd, int active)
{
	...
	if (active)
	{
		...
		*/
		...
		cmd->forwardmove *= EffectMoveScale.x;
		cmd->sidemove *= EffectMoveScale.y;
		cmd->upmove *= EffectMoveScale.z;

		//Master Sword - Add in arbitrary movement from effects (drunk, stun, etc.)
		Vector vTemp = Vector(cmd->forwardmove, cmd->sidemove, cmd->upmove);
		Vector EffectMove = gHUD.m_HUDScript->Effects_GetMove(vTemp);
		cmd->forwardmove = EffectMove.x;
		cmd->sidemove = EffectMove.y;
		cmd->upmove = EffectMove.z;

		//Master Sword - Check if any movement is allowed (Client or Server can set this flag)
		if (FBitSet(player.m_StatusFlags, PLAYER_MOVE_NOMOVE))
		{
			cmd->forwardmove *= 0;
			cmd->sidemove *= 0;
			cmd->upmove *= 0;
		...
		// clip to maxspeed
		spd = player.m_MaxSpeed; //gEngfuncs.GetClientMaxspeed();
		if (spd != 0.0)
		{
...
int CL_ButtonBits(int bResetState)
{
	...
	if (in_duck.state & 3)
	{
		if (!FBitSet(player.m_StatusFlags, PLAYER_MOVE_NODUCK))
			bits |= IN_DUCK;
	}

	if (in_jump.state & 3)
	{
		if (!FBitSet(player.m_StatusFlags, PLAYER_MOVE_NOJUMP))
			bits |= IN_JUMP;
	}
	...
	/*if ( CL_IsDead() || gHUD.m_iIntermission )
   {
	...
	if (bResetState)
	{
		...
		in_alt1.state &= ~2;
		in_score.state &= ~2;
		in_speed.state &= ~2; // MiB MAR2012_15 - +speed -> IN_RUN
	}
...
void InitInput(void)
{
	...
	//Thothie SEP2011_10 - intercept reconnect/quit attempts
	//No good as axes the commands entirely
	//gEngfuncs.pfnAddCommand ("quit",IN_BreakUp);
	//gEngfuncs.pfnAddCommand ("disconnect",IN_BreakUp);
	//gEngfuncs.pfnAddCommand ("retry",IN_BreakUp);
	...
	cl_forwardspeed = gEngfuncs.pfnRegisterVariable("cl_forwardspeed", "400", 0 /*FCVAR_ARCHIVE*/);
	cl_backspeed = gEngfuncs.pfnRegisterVariable("cl_backspeed", "400", 0 /*FCVAR_ARCHIVE*/);
