...
{
#ifdef DEV_BUILD
	MSTestView = !MSTestView;
#endif
}
Vector g_PunchAngles;
viewmgr_t ViewMgr;
...
void V_CalcGunAngle(struct ref_params_s *pparams)
{
	...
	//else ClearBits( viewent->curstate.oldbuttons, MSRDR_FLIPPED );
}
...
void V_CalcNormalRefdef(struct ref_params_s *pparams)
{
	...
	//MS: Save punchangles.
	//Laster, I remove the punchangles from the inset player model
	g_PunchAngles = Vector(ev_punchangle) + Vector(pparams->punchangle);
	...
	if (MSCLGlobals::CamThirdPerson)
	{
		...
		//pitch /= -3.0; //Master Sword UNDONE - Now player never tilts when normally looking up

		// Slam local player's pitch value
		if (ent->FullRotate())
		{
			//pitch = -pitch;							//Player tilts fully when playing the swim animation
			/*ent->angles[ 0 ] = pitch;
			ent->curstate.angles[ 0 ] = pitch;
			ent->prevstate.angles[ 0 ] = pitch;
			ent->latched.prevangles[ 0 ] = pitch;*/
			ent->curstate.fuser2 = -pitch / 3.0f; //Scale down from view angles - Now the pitch matches
												  //other players' pitches sent from the server
		}
		else
		{
			ent->angles[PITCH] = 0;
			ent->curstate.angles[PITCH] = 0;
			ent->prevstate.angles[PITCH] = 0;
			ent->latched.prevangles[PITCH] = 0;
			ent->curstate.fuser2 = pitch / 3.0f; //Scale down from view angles - Now the pitch matches
												 //other players' pitches sent from the server
		}
	}
	...
	if (MSTestView)
	{
		view->origin = Vector(0, 0, 72); // ms for testing the view model
		view->angles = Vector(0, 0, 0);
		view->curstate.angles = Vector(0, 0, 0);
	}

	//Master Sword - Let the client effects mess with the view
	gHUD.m_HUDScript->Effects_GetView(pparams, view);
...
void DLLEXPORT V_CalcRefdef(struct ref_params_s *pparams)
{
	...
	//Set the origin/angle of the inventory player model here
	//Must be done here, just after the view is calculated
	cl_entity_t &Ent = g_Inset.m_Ent;
	Ent.origin = pparams->vieworg;
	//EngineFunc::MakeVectors( pparams->viewangles, g_ViewForward, g_ViewRight, g_ViewUp );
	//Ent.origin += vForward * 4.7 /*+ vRight * 3.3*/ + vUp * -2.1;	<< - Full body, when using 0.02f scale

	//Ent.origin += g_ViewForward * 4.7 + g_ViewUp * -3.1;
	Ent.origin += pparams->forward * 4.7 + pparams->up * -3.1;

	Vector vAng = Vector(pparams->viewangles) /*- Vector( -g_PunchAngles.x, g_PunchAngles.y, g_PunchAngles.z )*/;
	Ent.angles = Vector(-vAng[0], vAng[1], 0) /*+ Vector(0,180,0)*/;
	Ent.curstate.angles = Ent.angles;

	//gEngfuncs.Cvar_SetValue( "lightgamma", CEnvMgr::m_LightGamma );
	pparams->movevars->zmax = CEnvMgr::m_MaxViewDistance;

	/*pparams->movevars->skycolor_r = 1;
	pparams->movevars->skycolor_g = 1;
	pparams->movevars->skycolor_b = 1;

	pparams->movevars->skyvec_x = 0;
