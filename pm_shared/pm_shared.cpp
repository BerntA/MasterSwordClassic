...
*
****/
//Master Sword - C++ version of the physics.
...
int PM_MapTextureTypeStepType(char chTextureType)
{
	switch (chTextureType)
	{
		...
		case CHAR_TEX_SNOW: return STEP_SNOW; //MAR2008a
	}
}
...
qboolean PM_CheckWater ()
{
	...
	if (cont <= CONTENTS_WATER && cont > CONTENTS_TRANSLUCENT )
	{
			 ...
			 ( truecont >= CONTENTS_CURRENT_DOWN ) )
		{
			...
			{
				Vector(1, 0, 0), Vector(0, 1, 0), Vector(-1, 0, 0),
...
void PM_CheckFalling( void )
{
	...
	if ( pmove->onground != -1 ) 
	{
		if( pmove->runfuncs && pmove->flFallVelocity )	//Call hitground event when landing a fall
		{
			if( PMScript )
			{
...
void PM_PlayerMove ( qboolean server )
{
	...
	case MOVETYPE_WALK:
		...
		if( PMScript )
		{
			PMScript->CallScriptEvent( "game_playermove" );
		}
...
msstring_ref PM_GetValue( msstringlist &Params )
{
	...
	if( Name == "fallvelocity" ) RETURN_FLOAT( pmove->flFallVelocity )
	else if( Name == "waterlevel" ) RETURN_INT( pmove->waterlevel )
	else if( Name == "oldwaterlevel" ) RETURN_INT( pmove->oldwaterlevel )
	else if( Name == "origin" ) RETURN_POSITION( "origin", pmove->origin )
	else if( Name == "angles" ) RETURN_ANGLE( "angles", pmove->angles )
	else if( Name == "velocity" ) RETURN_ANGLE( "velocity", pmove->velocity )
